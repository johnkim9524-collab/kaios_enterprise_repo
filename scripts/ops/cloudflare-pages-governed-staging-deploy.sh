...
if [[ "$SOURCE_DIR" != "apps/kidults-enterprise-staging/public" ]]; then
  echo "Refusing unexpected source directory: $SOURCE_DIR" >&2
  exit 64
fi
if [[ ! "$SOURCE_SHA" =~ ^[0-9a-f]{40}$ ]]; then
  echo "Exact source SHA is required" >&2
  exit 64
fi
if [[ -z "$DEPLOY_REASON" ]]; then
  echo "Deployment reason is required" >&2
  exit 64
fi
if [[ -z "${CLOUDFLARE_API_TOKEN:-}" || -z "${CLOUDFLARE_ACCOUNT_ID:-}" ]]; then
  echo "Cloudflare deployment credentials are absent" >&2
  exit 65
fi
...
if find "$SOURCE_DIR" -type l -print -quit | grep -q .; then
  echo "Symlinks are prohibited in the Pages upload directory" >&2
  exit 66
fi
if [[ -z "${GITHUB_RUN_ID:-}" || -z "${GITHUB_RUN_ATTEMPT:-}" || -z "${GITHUB_REPOSITORY:-}" ]]; then
  echo "GitHub run identity is required" >&2
  exit 66
fi
...
find "$SOURCE_DIR" -type f -print0 \
  ...
  | xargs -0 sha256sum \
  | sha256sum \
  | awk '{print "sha256:"$1}' > "$RECEIPT_DIR/source-tree.sha256"
source_tree_sha256="$(cat "$RECEIPT_DIR/source-tree.sha256")"
normalized_reason="$(python3 - <<'PY'
import os,re
value=re.sub(r'\s+', ' ', os.environ['DEPLOY_REASON']).strip()
if not value:
    raise SystemExit(64)
print(value[:160])
PY
...
if [[ "$deploy_rc" -ne 0 ]]; then
  cat "$RECEIPT_DIR/wrangler-deploy.log" >&2
  exit "$deploy_rc"
fi
...
jq --arg id "$new_id" '.result[] | select(.id == $id) | {
  id,
  environment,
  url,
  aliases: (.aliases // []),
...
}' "$tmp_dir/deployments-after.json" > "$RECEIPT_DIR/deployment.json"

test -s "$RECEIPT_DIR/deployment.json"
jq -e --arg sha "$SOURCE_SHA" --arg message "$commit_message" '
...
' "$RECEIPT_DIR/deployment.json" >/dev/null
...
jq -n \
  ...
  --arg deploy_reason "$normalized_reason" \
  --arg workflow_run_id "$GITHUB_RUN_ID" \
  --argjson workflow_run_attempt "$GITHUB_RUN_ATTEMPT" \
  --slurpfile deployment "$RECEIPT_DIR/deployment.json" \
  '{
    id:"kidults-cloudflare-governed-staging-deployment-receipt-v1",
    ...
    preview_deployment_created:false,
