  ...
  if (( status != 0 )) && [[ ! -s "$RECEIPT_DIR/final.json" ]]; then
    ...
    jq -n --arg reason_code "$failure_reason_code" --arg project "$PROJECT_NAME" \
      ...
      --argjson account_id_present "$account_id_present" --argjson api_call_attempted "$api_call_attempted" '{
        ...
        promotion_eligible:false,read_only:true,settings_mutated:false,deployment_created:false,deployment_deleted:false,
...
write_inventory_failure_receipt() {
  ...
  jq -n --arg state "$state" --arg reason_code "$reason_code" --arg project "$PROJECT_NAME" \
    ...
    --argjson partial_inventory_count "$partial_count" --argjson inventory_capacity "$inventory_capacity" '{
      ...
      terminal_receipt_preserved:true,read_only:true,settings_mutated:false,deployment_created:false,deployment_deleted:false,
      platform_environment:"STAGING",public_release:"HOLD",production:"HOLD",g5:"HOLD"
    }' > "$RECEIPT_DIR/final.json"
  cat "$RECEIPT_DIR/final.json"
  exit "$exit_code"
...
jq -n --arg state "$state" --arg project "$PROJECT_NAME" --arg current_main_sha "${GITHUB_SHA:-UNKNOWN}" \
  ...
  --slurpfile latest_attempt "$RECEIPT_DIR/latest-attempt.json" '{
    ...
    cloudflare_api_called:true,settings_readback_complete:true,deployment_inventory_complete:true,terminal_receipt_preserved:true,read_only:true,
