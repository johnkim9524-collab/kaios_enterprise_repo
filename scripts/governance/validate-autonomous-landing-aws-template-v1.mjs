...
for (const action of ['dynamodb:DescribeTable','dynamodb:GetItem','dynamodb:Query','lambda:InvokeFunction','lambda:GetFunction','kms:Sign','kms:DescribeKey','kms:GetKeyPolicy','kms:GetKeyRotationStatus','kms:ListResourceTags','dynamodb:DescribeContinuousBackups','dynamodb:ListTagsOfResource','s3:GetBucketObjectLockConfiguration','s3:GetBucketPublicAccessBlock','s3:GetBucketOwnershipControls','s3:GetEncryptionConfiguration','s3:GetBucketVersioning','s3:GetBucketTagging','s3:PutObject','s3:GetObject','s3:GetObjectAttributes','s3:GetObjectVersion','s3:GetObjectVersionAttributes','s3:GetObjectRetention','s3:PutObjectRetention','iam:GetRole','iam:GetRolePolicy','iam:ListAttachedRolePolicies','iam:ListRolePolicies','cloudformation:DetectStackDrift','cloudformation:DetectStackResourceDrift','cloudformation:DescribeStacks','cloudformation:DescribeStackDriftDetectionStatus','kms:Encrypt','kms:Decrypt','kms:GenerateDataKey']) assert.ok(finalizerActions.includes(action));
...
assert.deepEqual(iamReadback.Action, ['iam:GetRole','iam:GetRolePolicy','iam:ListAttachedRolePolicies','iam:ListRolePolicies']);
assert.equal(iamReadback.Resource.length, 5);
for (const roleName of ['finalizer','track','kpmo','verifier','ledger-writer']) assert.ok(iamReadback.Resource.some(value => value['Fn::Sub'].endsWith(`role/kidults-autonomous-${roleName}-staging-role`)));
const kmsDriftReadback = finalizerStatements.find(value => (value.Action || []).includes('kms:DescribeKey'));
assert.equal(kmsDriftReadback.Resource.length, 6);
const lambdaReadback = finalizerStatements.find(value => (value.Action || []).includes('lambda:GetFunction'));
assert.deepEqual(lambdaReadback.Action, ['lambda:InvokeFunction','lambda:GetFunction']);
