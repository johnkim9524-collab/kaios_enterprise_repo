...
const requiredSteps = ['IDENTIFY_ROOT_CAUSE','CORRECT_ROOT_CAUSE_IMMEDIATELY','ADD_OR_STRENGTHEN_REGRESSION_NEGATIVE_AND_MUTATION_TESTS','REVALIDATE_EXACT_HEAD','SYNC_CANONICAL_REGISTRY_ISSUES_AND_MANAGEMENT_TRUTH','REPORT_ONLY_AFTER_VERIFIED_REMEDIATION','PROPOSE_PRIORITIZED_CURRENT_IMPROVEMENTS','ASSESS_INTEGRATED_STRATEGY_EFFECT_WHEN_MATERIAL'];
...
const strategic = ['INTELLIGENCE_HOLDINGS_GROUP_FUTURE_STRATEGY','VERTICAL_BRAND_FUTURE_STRATEGY','CURRENT_PLATFORM_IMPROVEMENT_PROPOSALS','PRODUCT_STRATEGY','CUSTOMER_STRATEGY','PLATFORM_STRATEGY','PROVIDER_STRATEGY','INTERNALIZATION_STRATEGY','GO_TO_MARKET_AND_MONETIZATION'];
...
for (const key of ['KPMO','TRACK_A','TRACK_B','TRACK_C','TRACK_D','TRACK_E','TRACK_Z','ASI','RED_TEAM','EXTERNAL_MODEL_AGENTS']) if (seq.bootstrap_inheritance?.[key] !== true) fail(`SEQUENCE_INHERITANCE:${key}`);
...
for (const key of ['TRACK_A','TRACK_B','TRACK_C','TRACK_D','TRACK_E','TRACK_Z']) {
...
for (const a of ['PRODUCT_STRATEGY','CUSTOMER_STRATEGY','PLATFORM_STRATEGY','INTELLIGENCE_HOLDINGS_GROUP_FUTURE_STRATEGY','VERTICAL_AND_BRAND_FUTURE_STRATEGY']) if (!trackJd.tracks?.TRACK_E?.accountabilities?.includes(a)) fail(`TRACK_E:${a}`);
for (const a of ['PROVIDER_DISCOVERY_AND_PORTFOLIO_STRATEGY','INTERNALIZATION_CANDIDATE_IDENTIFICATION','BUILD_BUY_PARTNER_INTERNALIZE_RECOMMENDATION']) if (!trackJd.tracks?.TRACK_Z?.accountabilities?.includes(a)) fail(`TRACK_Z:${a}`);
...
for (const rule of ['NO_SINGLE_PROVIDER_AS_CANONICAL_TRUTH','PROOF_BEFORE_PROCUREMENT','RIGHTS_BEFORE_RECURRING_ACTIVATION','SHARED_PROVIDER_HEALTH_QUOTA_AND_CIRCUIT_STATE_FOR_SHARED_DEPENDENCIES','CODE_LANDING_OR_TEST_SUCCESS_NEVER_EQUALS_PROVIDER_ACTIVATION_AUTHORITY','GROUP_LEVEL_PROVIDER_REUSE_BEFORE_VERTICAL_DUPLICATE_PURCHASE']) if (!strategy.strategy_domains?.PROVIDER_STRATEGY?.portfolio_rules?.includes(rule)) fail(`PROVIDER_RULE:${rule}`);
for (const core of ['CANONICAL_IDENTITY_AND_ENTITY_RESOLUTION','PROVENANCE_AND_EVIDENCE_GRAPH','PROVIDER_ROUTING_FUSION_AND_SWITCHING','AI_AGENT_GOVERNANCE_AND_AUTONOMOUS_OPERATIONS']) if (!strategy.strategy_domains?.INTERNALIZATION_STRATEGY?.must_internalize?.includes(core)) fail(`INTERNALIZATION_CORE:${core}`);
for (const state of ['INTERNALIZE_NOW','HYBRID_OWNED_CORE_EXTERNAL_SOURCE','PARTNER_WITH_EXIT_PATH','KEEP_EXTERNAL_COMMODITY']) if (!strategy.strategy_domains?.INTERNALIZATION_STRATEGY?.decision_states?.includes(state)) fail(`INTERNALIZATION_STATE:${state}`);
...
for (const phrase of ['AUTONOMOUS','GLOBAL','IRREPLACEABLE VALUE','TRANSPARENT','REPORT_ONLY_AFTER_VERIFIED_REMEDIATION']) if (!doc.toUpperCase().includes(phrase)) fail(`DOC_CONTRACT:${phrase}`);
for (const phrase of ['Track A','Track B','Track C','Track D','Track E','Track Z','Intelligence Holdings','AUTONOMOUS','Internalization Strategy']) if (!trackDoc.includes(phrase)) fail(`TRACK_DOC_CONTRACT:${phrase}`);
for (const phrase of ['## 2. Product strategy','## 3. Customer strategy','## 4. Platform strategy','## 5. Provider strategy','## 6. Internalization strategy']) if (!strategyDoc.includes(phrase)) fail(`STRATEGY_DOC_CONTRACT:${phrase}`);
