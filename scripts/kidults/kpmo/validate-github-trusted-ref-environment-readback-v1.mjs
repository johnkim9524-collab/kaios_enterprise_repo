...
export function validateReceipt(receipt, { requireExternalProof = false } = {}) {
  ...
  require(receipt?.registered_privileged_manual_lanes === 14, 'registered_lane_count');
...
export function validateRepository(root = process.cwd()) {
  ...
  assert(inventory.registered_lane_count === registry.registered_count && registry.registered_count === 14, 'REGISTRY_LANE_PARTITION');
  assert(inventory.secret_bearing_job_count === 14, 'SECRET_BEARING_JOB_PARTITION');
  ...
  assert(testSource.includes('all 14 privileged jobs reject unreadable, stale, and non-main live-main guards'), 'PRIVILEGED_LIVE_MAIN_MUTATION_TEST_MISSING');
  assert(testSource.includes('all 14 privileged jobs reject secret scope and guard order mutations'), 'PRIVILEGED_SECRET_LIFETIME_MUTATION_TEST_MISSING');
  ...
  assert(docs.includes('14/14 guards') && docs.includes('zero workflow-scope bindings') && docs.includes('zero job-scope bindings'), 'DOC_PRIVILEGED_EXECUTION_COUNTS');
