...
export function validateRepository(root = process.cwd()) {
  ...
  assert(testSource.includes('all 16 privileged jobs reject unreadable, stale, and non-main live-main guards'), 'PRIVILEGED_LIVE_MAIN_MUTATION_TEST_MISSING');
  assert(testSource.includes('all 16 privileged jobs reject secret scope and guard order mutations'), 'PRIVILEGED_SECRET_LIFETIME_MUTATION_TEST_MISSING');
