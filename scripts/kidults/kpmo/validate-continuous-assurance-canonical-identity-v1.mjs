...
const contractText = normalizeNewlines(fs.readFileSync(contractPath, 'utf8'));
const contract = JSON.parse(contractText);
const workflow = normalizeNewlines(fs.readFileSync(workflowPath, 'utf8'));
...
const watchedWorkflowNames = (sourceText) => {
  const normalized = normalizeNewlines(sourceText);
  const workflowRunBlock = normalized.match(/^  workflow_run:\n([\s\S]*?)^    branches:/m)?.[1];
  assert(typeof workflowRunBlock === 'string', 'WORKFLOW_RUN_BLOCK_NOT_FOUND');
  return [...workflowRunBlock.matchAll(/^      - '([^']+)'$/gm)].map((match) => match[1]);
};
const watched = watchedWorkflowNames(workflow);
assert(JSON.stringify(watchedWorkflowNames(workflow.replaceAll('\n', '\r\n'))) === JSON.stringify(watched),
  'WORKFLOW_WATCH_CRLF_PARITY');
for (const entry of contract.workflow_run_class_allowlist) {
  ...
  const source = normalizeNewlines(fs.readFileSync(entry.workflow_path, 'utf8'));
...
const expectedClassByWorkflow = new Map([
  ...
  ['KIDULTS Autonomous Smithsonian Open Access Sample', 'ASI_ADAPTER_EVIDENCE_CASCADE'],
  ['KIDULTS Autonomous Art Institute Design Sample', 'ASI_ADAPTER_EVIDENCE_CASCADE'],
...
assert(contract.workflow_run_class_allowlist.filter((entry) => entry.upstream_class === 'ASI_ADAPTER_EVIDENCE_CASCADE').length === 9, 'ADAPTER_CLASS_CARDINALITY');
