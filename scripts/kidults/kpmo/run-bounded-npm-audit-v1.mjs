...
export function classifyAuditPayload(raw, exitCode = 0, timedOut = false) {
  if (timedOut) {
    return { valid: false, retryable: true, reason: 'TIMEOUT', exit_code: exitCode };
  }
  let payload;
  try {
    payload = JSON.parse(String(raw || ''));
  } catch {
    return { valid: false, retryable: true, reason: 'INVALID_JSON', exit_code: exitCode };
  }
  ...
  if (!countsValid) {
    return { valid: false, retryable: false, reason: 'INVALID_VULNERABILITY_COUNTS', exit_code: exitCode, payload };
  }
  return {
    valid: true,
    retryable: false,
    reason: 'VALID_AUDIT_RESPONSE',
    exit_code: exitCode,
    high,
    critical,
    payload
...
function safeName(lock) {
  const dir = path.dirname(lock);
  return dir.replace(/[/.]/g, '_') || '_';
}

function auditOne(lock) {
  const cwd = path.dirname(lock);
  const attempts = [];
  let final = null;
  for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt += 1) {
    ...
    if (classified.valid || !classified.retryable || attempt === MAX_ATTEMPTS) {
      final = classified;
      break;
    }
    sleep(attempt === 1 ? 2_000 : 5_000);
  }
  req(final, `AUDIT_FINAL_STATE_MISSING:${lock}`);
  return { lockfile: lock, attempts, final };
}
...
function selfTest() {
  ...
  console.log(JSON.stringify({
    test: 'BOUNDED_NPM_AUDIT_V1',
    state: 'VERIFIED_PASS',
    max_attempts: MAX_ATTEMPTS,
    attempt_timeout_ms: ATTEMPT_TIMEOUT_MS,
    transient_only_retry: true,
    vulnerability_response_not_retried: true,
...
function main() {
  ...
  const listed = spawnSync('git', ['ls-files', '*package-lock.json'], { encoding: 'utf8', maxBuffer: MAX_BUFFER });
  req(listed.status === 0, 'LOCKFILE_DISCOVERY_FAILED');
  const locks = String(listed.stdout || '').split('\n').map(x => x.trim()).filter(Boolean);
  const records = [];
  for (const lock of locks) {
    const record = auditOne(lock);
    records.push(record);
    const output = record.final?.payload ?? { error: { code: record.final?.reason || 'UNAVAILABLE_AFTER_RETRY' } };
    fs.writeFileSync(path.join(AUDIT_DIR, `${safeName(lock)}.json`), `${JSON.stringify(output, null, 2)}\n`);
    fs.appendFileSync(STATUS_PATH, `${JSON.stringify({
      lockfile: lock,
      attempts: record.attempts.length,
      final_exit_code: record.final.exit_code,
      valid_response: record.final.valid,
      retry_exhausted: !record.final.valid && record.final.retryable,
      final_reason: record.final.reason
    })}\n`);
  }
  ...
  let unavailable = 0;
  let invalid = 0;
  let anomalousNonzero = 0;
  let retryCount = 0;
  for (const record of records) {
    retryCount += Math.max(0, record.attempts.length - 1);
    if (!record.final.valid) {
      if (record.final.retryable) unavailable += 1;
      else invalid += 1;
      continue;
    }
  ...
  const summary = {
    audit_file_count: records.length,
    expected_lockfile_count: locks.length,
    high,
    critical,
    invalid,
    unavailable,
    anomalous_nonzero_exit: anomalousNonzero,
    retry_count: retryCount,
    max_attempts_per_lockfile: MAX_ATTEMPTS,
    attempt_timeout_ms: ATTEMPT_TIMEOUT_MS,
    npm_internal_fetch_retries: 0,
    retry_policy: 'TRANSIENT_INVALID_ERROR_OR_TIMEOUT_ONLY',
    scope: 'FULL_LOCKFILE_INCLUDING_DEV'
  };
  fs.writeFileSync(SUMMARY_PATH, `${JSON.stringify(summary, null, 2)}\n`);
  console.log(summary);
  if (records.length !== locks.length || invalid || unavailable || high || critical || anomalousNonzero) process.exit(1);
}
