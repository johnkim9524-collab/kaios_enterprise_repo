...
export function validateReadinessConsumption(receipt,{repository,runId,runAttempt,prNumber,headSha,now=Date.now(),maximumAgeSeconds=900,workflowName=null,workflowConclusion=null,workflowEvent=null,workflowHeadSha=null,workflowHeadBranch=null,workflowRepository=null,workflowPullRequests=null}){
  ...
  let boundPrNumber=prNumber==null||prNumber===''?null:Number(prNumber);
  let boundHeadSha=headSha==null||headSha===''?null:String(headSha);
  ...
  if(triggerPullRequests){
    const matched=triggerPullRequests.find(pr=>Number(pr?.number)===Number(receipt.pull_request)
      && String(pr?.head?.sha||pr?.head_sha||'')===String(receipt.exact_head_sha||''));
    if(!matched)fail('READINESS_WORKFLOW_RUN_PR_BINDING_MISMATCH');
    if(boundPrNumber==null) boundPrNumber=Number(matched.number);
    if(boundHeadSha==null) boundHeadSha=String(matched?.head?.sha||matched?.head_sha||'');
  }
  ...
  if(receipt.state==='LIFECYCLE_READY_AUTOMATED'){
    ...
    return {state:'READINESS_LIFECYCLE_ONLY_PENDING_RERUN',receipt_state:receipt.state,dispatch_eligible:false,pull_request:Number(boundPrNumber),exact_head_sha:boundHeadSha,production:'HOLD',public_release:'HOLD',g5:'HOLD'};
  }
  if(TERMINAL_NONCONSUMABLE_STATES.has(receipt.state)){
    ...
    if(receipt.production!=='HOLD'||receipt.public_release!=='HOLD'||receipt.g5!=='HOLD')fail('READINESS_RECEIPT_HOLD_BOUNDARY_INVALID');
    return {state:'READINESS_NONCONSUMABLE_TERMINAL',receipt_state:receipt.state,dispatch_eligible:false,pull_request:Number(boundPrNumber),exact_head_sha:boundHeadSha,production:'HOLD',public_release:'HOLD',g5:'HOLD'};
  }
  ...
  return {state:'READINESS_EXACT_HEAD_CONSUMED_FOR_AUTONOMOUS_DISPATCH',dispatch_eligible:true,pull_request:Number(boundPrNumber),exact_head_sha:boundHeadSha,age_seconds:age,failure_code:null,production:'HOLD',public_release:'HOLD',g5:'HOLD'};
}
