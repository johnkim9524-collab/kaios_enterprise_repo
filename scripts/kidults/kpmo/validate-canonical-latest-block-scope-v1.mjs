...
function selfTest(){
 ...
 console.log(JSON.stringify({state:'VERIFIED_PASS',historical_fallback_rejected:true,stale_main_rejected:true,hold_required:true}));
}
...
if(failures.length){console.error(JSON.stringify({state:'VERIFIED_FAIL',expected_main_sha:expectedMainSha,failures,promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));process.exit(1);}
console.log(JSON.stringify({state:'VERIFIED_PASS',expected_main_sha:expectedMainSha,canonical_issue_count:issues.length,active_baseline_defects:active,latest_block_only:true,historical_fallback_rejected:true,stale_main_rejected:true,hold_required:true,promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));
