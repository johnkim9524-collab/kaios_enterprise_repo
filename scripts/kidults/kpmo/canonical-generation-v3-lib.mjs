...
// Separate legitimate material drift from immutable identity/authority drift.
// This comparison is diagnostic, never by itself permission to refresh or PASS.
export function snapshotMismatchFields(payload,snapshot){
  const actual=snapshotProjection(payload),expected=snapshotProjection(snapshot);
  return Object.keys(expected).filter(key=>stableJson(actual[key])!==stableJson(expected[key]));
}
export function validateShared(payload,snapshot,label){
  const fields=snapshotMismatchFields(payload,snapshot);
  if(fields.length){const error=new Error(`${label}_MISMATCH`);error.mismatch_fields=fields;throw error;}
  if(payload.production!=='HOLD'||payload.public!=='HOLD'||payload.g5!=='HOLD'||payload.promotion_eligible!==false)throw new Error(`${label}_HOLD_INVALID`);
}

// Only a separately authorized writer may use this historical projection, and
// only after verifying the immutable aggregate, all 25 members and native run.
// No old payload is relabeled current and no old approval is transferred.
export function validateMaterialRefreshProjection(commit,snapshot){
  const allowed=new Set(['active_baseline_defects','material_defect_count','material_defect_issue_numbers','material_defect_registry_sha256','truth_digest']);
  const fields=snapshotMismatchFields(commit,snapshot);
  if(!fields.length||fields.some(key=>!allowed.has(key)))throw new Error('REFRESH_NON_MATERIAL_DRIFT');
  const ordered=value=>Array.isArray(value)&&value.every((n,i)=>Number.isSafeInteger(n)&&n>0&&(i===0||value[i-1]<n));
  if(commit.version!=='3.1.0'||!Number.isSafeInteger(commit.writer_run_id)||commit.writer_run_id<1||commit.writer_run_attempt!==1)throw new Error('REFRESH_PRIOR_VERSION_OR_RUN_INVALID');
  if(!ordered(commit.material_defect_issue_numbers)||!Number.isSafeInteger(commit.material_defect_count)||commit.material_defect_count!==commit.material_defect_issue_numbers.length)throw new Error('REFRESH_PRIOR_MATERIAL_SET_INVALID');
  if(!ordered(commit.active_baseline_defects)||commit.active_baseline_defects.some(n=>!BASELINE.includes(n)))throw new Error('REFRESH_PRIOR_BASELINE_INVALID');
  for(const key of ['material_defect_registry_sha256','truth_digest'])if(!/^sha256:[0-9a-f]{64}$/.test(commit[key]||''))throw new Error('REFRESH_PRIOR_DIGEST_INVALID');
  if(typeof commit.committed_at!=='string'||!Number.isFinite(Date.parse(commit.committed_at)))throw new Error('REFRESH_PRIOR_TIME_INVALID');
  const expected=commitPayload(commit,commit.generation_id,commit.writer_run_id,commit.writer_run_attempt,commit.member_comments,commit.committed_at);
  exact(Object.keys(commit).sort(),Object.keys(expected).sort(),'REFRESH_PRIOR_FIELDS');
  validateCommit(commit,commit);
  return fields;
}
export function classifyIssueRegistrySnapshot(commit,snapshot){
  const fields=snapshotMismatchFields(commit,snapshot);
  if(!fields.length)return {stale:false,stale_reason:null,mismatch_fields:[]};
  return {stale:true,stale_reason:'ISSUE_REGISTRY_SNAPSHOT_MOVED',mismatch_fields:validateMaterialRefreshProjection(commit,snapshot)};
}
...
export function validateCommitEnvelope(payload,run=null){
  if(payload.schema!=='kpmo-canonical-generation-v3-commit'||payload.version!=='3.1.0'||payload.state!=='COMMITTED'||payload.aggregate_issue_number!==AGGREGATE||payload.writer_workflow_path!==WRITER_WORKFLOW||!Number.isSafeInteger(payload.writer_run_id)||payload.writer_run_id<1||payload.writer_run_attempt!==1||!Array.isArray(payload.member_comments)||payload.member_comments.length!==MEMBERS.length)throw new Error('COMMIT_PAYLOAD_INVALID');
  if(typeof payload.committed_at!=='string'||!Number.isFinite(Date.parse(payload.committed_at)))throw new Error('COMMIT_TIME_INVALID');
  const expected=commitPayload(payload,payload.generation_id,payload.writer_run_id,payload.writer_run_attempt,payload.member_comments,payload.committed_at);
  exact(Object.keys(payload).sort(),Object.keys(expected).sort(),'COMMIT_FIELDS');
  ...
  for(const entry of payload.member_comments)exact(Object.keys(entry).sort(),['comment_body_sha256','comment_id','issue_number'],'COMMIT_MEMBER_FIELDS');
  if(new Set(payload.member_comments.map((entry)=>entry.comment_id)).size!==MEMBERS.length||payload.member_comments.some((entry)=>!Number.isSafeInteger(entry.comment_id)||entry.comment_id<1||!/^sha256:[0-9a-f]{64}$/.test(entry.comment_body_sha256||'')))throw new Error('COMMIT_MEMBER_ID_OR_DIGEST_INVALID');
  exact(payload.canonical_issue_numbers,MEMBERS,'COMMIT_CANONICAL_ISSUE_ORDER');
  const ordered=value=>Array.isArray(value)&&value.every((number,index)=>Number.isSafeInteger(number)&&number>0&&(index===0||value[index-1]<number));
  if(payload.canonical_issue_count!==MEMBERS.length||!ordered(payload.active_baseline_defects)||payload.active_baseline_defects.some(number=>!BASELINE.includes(number))||!Number.isSafeInteger(payload.material_defect_count)||payload.material_defect_count<0||!ordered(payload.material_defect_issue_numbers)||payload.material_defect_issue_numbers.length!==payload.material_defect_count||!/^sha256:[0-9a-f]{64}$/.test(payload.material_defect_registry_sha256||'')||!/^sha256:[0-9a-f]{64}$/.test(payload.truth_digest||''))throw new Error('COMMIT_SNAPSHOT_PROJECTION_INVALID');
...
export function selfTest(){
  ...
  const unchanged=classifyIssueRegistrySnapshot(commit,snapshot);
  if(unchanged.stale||unchanged.stale_reason!==null||unchanged.mismatch_fields.length)throw new Error('SELF_TEST_UNCHANGED_SNAPSHOT_NOT_IDEMPOTENT');
  const moved=(changes)=>{const value={...snapshot,...changes};const {truth_digest:_priorTruthDigest,...payload}=value;value.truth_digest=sha256(payload);return value;};
  const registryMoves=[
    moved({material_defect_count:2,material_defect_issue_numbers:[9,10],material_defect_registry_sha256:`sha256:${'2'.repeat(64)}`}),
    moved({material_defect_count:0,material_defect_issue_numbers:[],material_defect_registry_sha256:`sha256:${'3'.repeat(64)}`}),
    moved({material_defect_registry_sha256:`sha256:${'4'.repeat(64)}`}),
    moved({active_baseline_defects:[1330,1412]}),
  ];
  for(const current of registryMoves){const classification=classifyIssueRegistrySnapshot(commit,current);if(!classification.stale||classification.stale_reason!=='ISSUE_REGISTRY_SNAPSHOT_MOVED')throw new Error('SELF_TEST_REGISTRY_MOVE_NOT_STALE');}
  for(const [name,fn] of [
    ...
    ['canonical-membership',()=>validateCommit({...commit,canonical_issue_numbers:MEMBERS.slice(1)},snapshot)],
    ['committed-time',()=>validateCommit({...commit,committed_at:'invalid'},snapshot)],
    ['member-extra-field',()=>validateCommit({...commit,member_comments:commit.member_comments.map((entry,index)=>index===0?{...entry,approved:true}:entry)},snapshot)],
  ...
  return {test:'KPMO_CANONICAL_GENERATION_V3_SELF_TEST',state:'VERIFIED_PASS',canonical_members:25,append_only_payload_model:true,partial_generation_nonauthoritative:true,exact_main_and_hold:true,material_registry_bound:true,same_main_issue_registry_snapshot_moved:true,unchanged_snapshot_idempotent_eligible:true,workflow_bootstrap_read_only:true,writes_reachable_from_workflow:false,workflow_roles_separated:true,writer_workflow_path:WRITER_WORKFLOW,mutations_rejected:9};
}
