...
const success = ids.map((id) => ({ id, name: id, outcome: 'success' }));
...
try { reconcileReceipt(base, 'success', reserveFailure); } catch (error) { inconsistentRejected = String(error.message).includes('SUCCESS_JOB_WITH_NON_SUCCESS_REQUIRED_STEP'); }
...
console.log(JSON.stringify({ suite: 'KIDULTS_CONTINUOUS_ASSURANCE_INLINE_TERMINAL_RECONCILIATION_V1', positive: 3, negative: 1, state: 'VERIFIED_PASS' }));
