...
export function classifyCandidate({pr,mainSha,treeSha,files,statuses=[],checks=[],policy,now=new Date()}) {
  ...
  const cleanStatuses=statuses.map(x=>({context:String(x.context),state:String(x.state)})).sort((a,b)=>a.context.localeCompare(b.context));
  const cleanChecks=checks.map(x=>({name:String(x.name),status:String(x.status),conclusion:String(x.conclusion)})).sort((a,b)=>a.name.localeCompare(b.name));
  ...
  const testEvidence={source:'GITHUB_LIVE_REQUIRED_CHECKS',result:'PASS',statuses:cleanStatuses,checks:cleanChecks};
...
export async function discover({repository,token,prNumber,policy}){
  ...
  const branch=await api(`/repos/${repository}/branches/main`,token); const mainSha=branch.commit?.sha;
  ...
  for(const pr of prs){try{
    ...
    results.push({state:'ELIGIBLE',envelope:classifyCandidate({pr,mainSha,treeSha:commit.tree?.sha,files,statuses:status.statuses||[],checks:checkData.check_runs||[],policy})});
