...
export function classifyAssuranceSemantics(jobs, mergeSha, expectedRunId) {
  ...
  if (conclusions.every(value => value === 'success')) {
    return {
      ok: true,
      state: 'ASSURANCE_BINDING_STEPS_SUCCESS_CONTENT_UNATTESTED',
...
async function selfTest() {
  ...
  assert.equal(authoritative.producer_health_authority, false);
  assert.equal(authoritative.artifact_content_validated, false);
...
async function main() {
  ...
  const request = async apiPath => {
    const response = await fetch(`https://api.github.com/repos/${repository}${apiPath}`, {headers, redirect: 'error', signal: AbortSignal.timeout(20000)});
  ...
  const finalEvaluation = evaluatePostMergePushSuite(await readRuns(), policy, mergeSha, receipt.merged_at);
  const generation = value => value.required.map(({path, run_id, run_attempt, conclusion, created_at}) =>
    [path, run_id, run_attempt, conclusion, created_at]);
  requireCondition(finalEvaluation.ready && JSON.stringify(generation(finalEvaluation)) === JSON.stringify(generation(evaluation)),
    'DIRECT_OWNER_POSTMERGE_RUN_GENERATION_CHANGED');
  const finalMain = await request('/branches/main');
  requireCondition(finalMain?.commit?.sha === mergeSha, 'DIRECT_OWNER_POSTMERGE_MAIN_ADVANCED_DURING_CONSUMPTION');
  ...
  const postMergeProof = {
    ...
    // Job/step outcomes are not producer payload evidence, even when all succeed.
    producer_health_authority: false,
