...
export function validatePlan(plan){
  req(plan?.id==='kidults-canonical-issue-rebound-plan-v2','PLAN_ID');
  req(plan?.version==='2.0.0','PLAN_VERSION');
  ...
  req(JSON.stringify(plan.issues.map(x=>x.issue_number))===JSON.stringify(canonicalIssues),'PLAN_ISSUE_ORDER');
...
async function controlState({repository,token,expectedMainSha}){
  ...
  return {headers,observedMainSha,material:summary};
}
...
export async function createPlan({repository,token,expectedMainSha,authorizationId,actor}){
  ...
  const block=buildCanonicalBlock({mainSha:expectedMainSha,authorizationId,material:state.material});
  const issues=[];
  ...
  const plan={
    id:'kidults-canonical-issue-rebound-plan-v2',version:'2.0.0',created_at:new Date().toISOString(),repository,
    ...
    authorization_id:authorizationId,actor,material_defect_count:state.material.count,
    material_defect_registry_binding_sha256:state.material.digest,material_defect_registry_digest_scope:state.material.digest_scope,
    material_defect_registry_members:state.material.members,canonical_block:block,issues,
    authority_boundary:{issue_body_append_planned:true,issue_write_performed:false,empirical_promotion:false,public:'HOLD',production:'HOLD',g5:'HOLD'},
  ...
  plan.plan_digest=digestObject(planCore(plan));
  validatePlan(plan);
  return plan;
}
...
export async function applyPlan({plan,token,actor,confirmApply,receiptPath}){
  ...
  const receipt={id:'kidults-canonical-issue-rebound-terminal-v2',version:'2.0.0',started_at:new Date().toISOString(),repository:plan.repository,plan_digest:plan.plan_digest,expected_main_sha:plan.expected_main_sha,authorization_id:plan.authorization_id,transaction_semantics:plan.transaction_semantics,state:'VERIFIED_FAIL_UNRECONCILED',applied_issue_numbers:[],postread_issue_numbers:[],rollback:[],promotion_eligible:false,public:'HOLD',production:'HOLD',g5:'HOLD',residual_risk:plan.residual_risk};
  let headers=null;
  try{
    ...
    for(const item of plan.issues){
      ...
      const failures=validateCanonicalBlock(issue.body||'',plan.expected_main_sha,{count:plan.material_defect_count,digest:plan.material_defect_registry_binding_sha256,members:plan.material_defect_registry_members});
      req(failures.length===0,`POSTREAD_CANONICAL_BINDING:${item.issue_number}:${failures.join('|')}`);
    ...
    receipt.state='VERIFIED_PASS_25_OF_25_APPEND_AND_READBACK';
    receipt.completed_at=new Date().toISOString();receipt.issue_write_performed=true;receipt.rollback_required=false;
...
function selfTest(){
  const main='a'.repeat(40),authorizationId=expectedAuthorization(main);
  const material={count:2,digest:`sha256:${'c'.repeat(64)}`,members:[10,11],digest_scope:'MATERIAL_DEFECT_REGISTRY_V3_STABLE_FIELDS'};
  const block=buildCanonicalBlock({mainSha:main,authorizationId,material});
  ...
  const plan={id:'kidults-canonical-issue-rebound-plan-v2',version:'2.0.0',transaction_semantics:'REVERSIBLE_FAIL_CLOSED_NOT_GITHUB_ATOMIC',expected_main_sha:main,observed_main_sha:main,authorization_id:authorizationId,actor:'johnkim9524-collab',material_defect_count:2,material_defect_registry_binding_sha256:material.digest,material_defect_registry_members:[10,11],canonical_block:block,issues};
  ...
  process.stdout.write(`${JSON.stringify({test:'CANONICAL_ISSUE_REBOUND_V2',state:'VERIFIED_PASS',append_only:true,shared_material_registry_parser:true,preimage_digest:true,plan_digest:true,rollback_packet_required_before_apply:true,github_atomicity_claimed:false})}\n`);
}
