...
if (!errors.length) {
  ...
  const requiredWorkflowMarkers = [
    ...
    'source_failure_dominates_generic_audit_pass'
  ];
  ...
  for (const marker of ['WORKFLOW_JOB_FAILURE_UNATTRIBUTED', 'WORKFLOW_TERMINAL_RECONCILIATION_FATAL', 'source_failure_dominates_generic_audit_pass', 'required_step_outcomes', 'causal_receipts', 'REQUIRED_STEP_APPLICABILITY_INVALID', 'receipt_digest', 'promotion_eligible: false']) {
    if (!terminalReconciler.includes(marker)) errors.push(`terminal reconciler marker missing: ${marker}`);
  }
  for (const marker of ['WORKFLOW_REQUIRED_STEP_EPHEMERAL_GUARD', 'WORKFLOW_REQUIRED_STEP_RESERVE_BINDING', 'WORKFLOW_REQUIRED_STEP_TRUTH_BINDING', 'TERMINAL_FAILURE_NOT_PROPAGATED_TO_REMEDIATION', 'CAUSAL_RECEIPT_TAMPER_ACCEPTED', 'UNATTRIBUTED_FAILURE', 'INCONSISTENT_SUCCESS_ACCEPTED', 'EXPECTED_NON_APPLICABLE_SKIP', 'DIGEST_BINDING']) {
