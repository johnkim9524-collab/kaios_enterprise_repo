...
if(!/^[0-9a-f]{40}$/i.test(expectedBodyMainSha||'')) fail('EXPECTED_CANONICAL_BODY_MAIN_SHA must be an exact SHA');
if(!['TRANSITION','SYNCHRONIZED'].includes(truthPhase)) fail('CANONICAL_TRUTH_PHASE must be TRANSITION or SYNCHRONIZED');
...
function validateBodies(requiredBodySha,correctionHead,issues,activeDefects,enforceCorrectionHead){
  const errors=[];
  for(const issue of issues){
    ...
    else if(recorded!==requiredBodySha) errors.push(`#${issue.number} canonical protected-main SHA ${recorded} != required generation ${requiredBodySha}`);
...
if(truthPhase==='TRANSITION'){
  if(!/^[0-9a-f]{40}$/i.test(observedParentSha)) fail('live protected-main first parent is unavailable');
  if(expectedBodyMainSha!==observedParentSha) fail(`transition generation ${expectedBodyMainSha} != exact first parent ${observedParentSha}`);
  if(expectedBodyMainSha===expectedMainSha) fail('transition generation must differ from current protected main');
}else if(expectedBodyMainSha!==expectedMainSha){
  fail(`synchronized generation ${expectedBodyMainSha} != current protected main ${expectedMainSha}`);
}
...
const errors=validateBodies(expectedBodyMainSha,correctionHead,issues,activeDefects,requireLiveCorrectionHead); if(errors.length) fail(errors.join('; '));

const staleGenerationMutation=structuredClone(issues); const firstRecorded=canonicalMainSha(staleGenerationMutation[0].body);
staleGenerationMutation[0].body=staleGenerationMutation[0].body.replace(firstRecorded,'1111111111111111111111111111111111111111');
if(!validateBodies(expectedBodyMainSha,correctionHead,staleGenerationMutation,activeDefects,requireLiveCorrectionHead).length) fail('wrong-generation mutation was not rejected');
const correctionMutation=structuredClone(issues); correctionMutation[0].body=`${correctionMutation[0].body}\n#${correctionPrNumber} exact head ${correctionHead}\n`.replaceAll(correctionHead,'1111111111111111111111111111111111111111'); if(!validateBodies(expectedBodyMainSha,correctionHead,correctionMutation,activeDefects,true).length) fail('stale-correction-head mutation was not rejected');
if(activeDefects.length){ const omissionMutation=structuredClone(issues); omissionMutation[0].body=omissionMutation[0].body.replaceAll(`#${activeDefects[0]}`,''); if(!validateBodies(expectedBodyMainSha,correctionHead,omissionMutation,activeDefects,requireLiveCorrectionHead).length) fail('active-defect omission mutation was not rejected'); }
...
for(const mutationText of closureMutationTexts){ const closureMutation=structuredClone(issues); closureMutation[0].body+=`\n${mutationText}\n`; if(!validateBodies(expectedBodyMainSha,correctionHead,closureMutation,activeDefects,requireLiveCorrectionHead).length) fail(`unsupported-closure mutation was not rejected: ${mutationText}`); }
console.log(JSON.stringify({validator:'LIVE_CANONICAL_ISSUE_TRUTH_V1',state:'VERIFIED_PASS',truth_phase:truthPhase,protected_main_sha:expectedMainSha,canonical_body_main_sha:expectedBodyMainSha,live_main_observed:observedMainSha,live_main_first_parent:observedParentSha||null,main_policy:truthPhase==='TRANSITION'?'EXACT_IMMEDIATE_PRIOR_GENERATION':'EXACT_CURRENT_MAIN',canonical_correction_pr:correctionPrNumber,canonical_correction_head:correctionHead,live_correction_head_enforced_in_issues:requireLiveCorrectionHead,canonical_issues:canonicalIssues,active_defects:activeDefects,github_read_mode:'SINGLE_GRAPHQL_BATCH',wrong_generation_mutation_rejected:true,stale_correction_head_mutation_rejected:true,active_defect_omission_mutation_rejected:activeDefects.length>0,unsupported_closure_mutations_rejected:closureMutationTexts.length,empirical_promotion:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));
