...
async function graphql(){
 ...
 const q=`query($owner:String!,$name:String!,$correction:Int!){repository(owner:$owner,name:$name){ref(qualifiedName:\"refs/heads/main\"){target{... on Commit{oid parents(first:1){nodes{oid}} history(first:3){nodes{oid}}}}} pullRequest(number:$correction){headRefOid baseRefName state merged} ${issues}}}`;
...
else if(truthPhase==='PREMERGE')allowed=new Set([observedMain,parent].filter(isSha));
...
const newest=set.reduce((best,g)=>best===null||history.indexOf(g)<history.indexOf(best)?g:best,null);if(process.env.GITHUB_OUTPUT)fs.appendFileSync(process.env.GITHUB_OUTPUT,`canonical_body_main_sha=${newest}\n`);
console.log(JSON.stringify({validator:'LIVE_CANONICAL_ISSUE_TRUTH_V1',state:'VERIFIED_PASS',truth_phase:truthPhase,validation_event:validationEvent,protected_main_sha:observedMain,canonical_generations:set,allowed_generation_window:[...allowed],canonical_body_main_sha:newest,live_main_parent:parent||null,live_main_grandparent:grandparent||null,canonical_correction_pr:correctionPrNumber,previous_correction_pr:previousCorrectionPrNumber,canonical_correction_head:correctionHead,canonical_issues:canonicalIssues,active_defects:active,truth_source_policy:'LATEST_TRUSTED_APPEND_ONLY_RECEIPT_V3_ELSE_LEGACY_BODY_V2',github_read_mode:'SINGLE_GRAPHQL_BATCH',transition_policy:'BOUNDED_ADJACENT_GENERATIONS',synchronized_policy:'EXACT_CURRENT_GENERATION_AND_CURRENT_CORRECTION',out_of_window_generation_mutation_rejected:true,stale_correction_mutation_rejected:true,active_defect_omission_mutation_rejected:active.length>0,false_closure_mutation_rejected:true,promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));
