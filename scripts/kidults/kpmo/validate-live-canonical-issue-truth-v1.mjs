...
const history=(live.ref?.target?.history?.nodes||[]).map(n=>n.oid).filter(isSha);
...
if(truthPhase==='TRANSITION'){if(!isSha(parent))fail('main parent unavailable');if(bodyMain!==parent)fail(`TRANSITION requires immediate parent ${parent}, observed ${bodyMain}`);}
