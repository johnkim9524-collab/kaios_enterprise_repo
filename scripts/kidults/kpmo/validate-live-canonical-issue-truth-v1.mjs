...
async function graphql(){
 ...
 const q=`query($owner:String!,$name:String!,$correction:Int!,$expected:String!){repository(owner:$owner,name:$name){ref(qualifiedName:\"refs/heads/main\"){target{... on Commit{oid parents(first:1){nodes{oid}} history(first:20){nodes{oid}}}}} expectedBase:object(expression:$expected){... on Commit{oid parents(first:1){nodes{oid parents(first:1){nodes{oid}}}}}} pullRequest(number:$correction){headRefOid baseRefName state merged} ${issues}}}`;
 const r=await fetch('https://api.github.com/graphql',{method:'POST',headers:{Accept:'application/vnd.github+json',Authorization:`Bearer ${token}`,'Content-Type':'application/json','X-GitHub-Api-Version':'2022-11-28'},body:JSON.stringify({query:q,variables:{owner,name,correction:correctionPrNumber,expected:expectedMainSha}}),signal:AbortSignal.timeout(15000)});
...
const expectedBase=live.expectedBase,expectedBaseOid=expectedBase?.oid||'';
if(truthPhase==='PREMERGE'&&expectedBaseOid!==expectedMainSha)fail(`PREMERGE exact event base object unavailable: expected ${expectedMainSha}, observed ${expectedBaseOid||'NONE'}`);
if(truthPhase==='PREMERGE'&&!history.includes(expectedMainSha))fail(`PREMERGE event base ${expectedMainSha} is no longer in bounded live-main history`);
const expectedBaseParent=expectedBase?.parents?.nodes?.[0]?.oid||'';
const expectedBaseGrandparent=expectedBase?.parents?.nodes?.[0]?.parents?.nodes?.[0]?.oid||'';
...
else if(truthPhase==='PREMERGE')allowed=new Set([expectedMainSha,expectedBaseParent,expectedBaseGrandparent].filter(isSha));
...
console.log(JSON.stringify({validator:'LIVE_CANONICAL_ISSUE_TRUTH_V1',state:'VERIFIED_PASS',truth_phase:truthPhase,validation_event:validationEvent,protected_main_sha:observedMain,expected_event_main_sha:expectedMainSha,premerge_event_base_parent:expectedBaseParent||null,premerge_event_base_grandparent:expectedBaseGrandparent||null,canonical_generations:set,allowed_generation_window:[...allowed],canonical_body_main_sha:newest,live_main_parent:parent||null,live_main_grandparent:grandparent||null,canonical_correction_pr:correctionPrNumber,previous_correction_pr:previousCorrectionPrNumber,canonical_correction_head:correctionHead,canonical_issues:canonicalIssues,active_defects:active,truth_source_policy:'LATEST_TRUSTED_APPEND_ONLY_RECEIPT_V3_ELSE_LEGACY_BODY_V2',github_read_mode:'SINGLE_GRAPHQL_BATCH',premerge_policy:'EVENT_BASE_MAINLINE_FIRST_PARENT_DEPTH_2_WITH_LIVE_HISTORY_MEMBERSHIP',transition_policy:'BOUNDED_ADJACENT_GENERATIONS',synchronized_policy:'EXACT_CURRENT_GENERATION_AND_CURRENT_CORRECTION',out_of_window_generation_mutation_rejected:true,stale_correction_mutation_rejected:true,active_defect_omission_mutation_rejected:active.length>0,false_closure_mutation_rejected:true,promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));
