...
ok(auth.consumption_result?.workflow_run_id === 33465807642, 'CONSUMING_RUN');
ok(auth.consumption_result?.job_id === 99725309548, 'CONSUMING_JOB');
ok(auth.consumption_result?.source_sha === 'b467787d358b85968ebfe7d993a538faa8b70e13', 'CONSUMING_SHA');
ok(auth.consumption_result?.authorization_consumed === true, 'AUTH_NOT_CONSUMED');
ok(auth.consumption_result?.unique_first_dispatch_verified === true, 'UNIQUE_FIRST_DISPATCH');
ok(auth.consumption_result?.locked_wrangler_dry_run_verified === true, 'DRY_RUN');
ok(auth.consumption_result?.dry_run_asset_count === 128, 'DRY_RUN_ASSET_COUNT');
ok(auth.consumption_result?.provider_attempt_marker_written === true, 'PROVIDER_MARKER');
ok(auth.consumption_result?.provider_process_invoked === true, 'PROVIDER_PROCESS');
ok(auth.consumption_result?.provider_deployment_attempt_count === 1, 'PROVIDER_ATTEMPT_COUNT');
ok(auth.consumption_result?.provider_exit_code === 1, 'PROVIDER_EXIT');
ok(auth.consumption_result?.remote_api_request_evidenced === true, 'REMOTE_API_REQUEST');
ok(auth.consumption_result?.cloudflare_error_code === 7003, 'CLOUDFLARE_ERROR_CODE');
ok(auth.consumption_result?.root_cause_class === 'CLOUDFLARE_ACCOUNT_ID_OR_TOKEN_ACCOUNT_SCOPE_MISMATCH', 'ROOT_CAUSE_CLASS');
ok(auth.consumption_result?.worker_upload_completed === false, 'UPLOAD_TRUTH');
ok(auth.consumption_result?.worker_deployment_success === false, 'DEPLOYMENT_TRUTH');
ok(auth.consumption_result?.workers_dev_url === null, 'WORKERS_DEV_URL_TRUTH');
ok(auth.consumption_result?.readback_executed === false, 'READBACK_TRUTH');
ok(auth.consumption_result?.remote_mutation_evidenced === false, 'REMOTE_MUTATION_TRUTH');
ok(auth.consumption_result?.artifact_id === 9784793397, 'ARTIFACT_ID');
ok(auth.consumption_result?.artifact_digest === 'sha256:0ab4517f47cfbb2cdf3de1a81e03af981df2dd5285df403dc8b4f611c5267c05', 'ARTIFACT_DIGEST');
...
ok(auth.future_execution?.environment_secret_correction_required === true, 'SECRET_CORRECTION');
...
ok(terminal.artifact?.digest === 'sha256:0ab4517f47cfbb2cdf3de1a81e03af981df2dd5285df403dc8b4f611c5267c05', 'TERMINAL_ARTIFACT_DIGEST');
...
ok(uploadIndex >= 0 && redIndex > uploadIndex, 'WORKFLOW_UPLOAD_BEFORE_RED');
for (const forbidden of [
  'environment:', '${{ secrets.', 'actions/checkout@', 'actions/setup-node@',
  'curl ', 'npm ', 'npx ', 'wrangler ', 'CLOUDFLARE_API_TOKEN', 'CLOUDFLARE_ACCOUNT_ID'
]) ok(!workflow.includes(forbidden), `WORKFLOW_EXECUTABLE_AUTHORITY:${forbidden}`);

ok(!registry.registered_workflows?.includes(WORKFLOW_PATH), 'REGISTRY_V3_PRESENT');
ok(!registry.required_environment_bindings?.some(binding => binding.workflow === WORKFLOW_PATH), 'REGISTRY_V3_BINDING_PRESENT');
ok(registry.registered_count === 22, 'REGISTRY_COUNT');
ok(registry.registered_workflows?.length === 22, 'REGISTRY_WORKFLOW_LENGTH');
ok(registry.required_environment_bindings?.length === 22, 'REGISTRY_BINDING_LENGTH');
for (const key of [
...
ok(preflight.status === 'MANDATORY_BEFORE_ANY_FUTURE_WORKERS_SHADOW_DEPLOYMENT_APPROVAL', 'PREFLIGHT_STATUS');
ok(preflight.authority?.standing_execution_authority === false, 'PREFLIGHT_STANDING_AUTHORITY');
ok(preflight.authority?.separate_explicit_program_owner_approval_required === true, 'PREFLIGHT_APPROVAL_REQUIRED');
ok(preflight.authority?.read_only_external_calls_only === true, 'PREFLIGHT_READ_ONLY');
ok(preflight.authority?.worker_mutation_allowed === false, 'PREFLIGHT_MUTATION');
...
ok(preflight.maximum_external_read_requests === 3, 'PREFLIGHT_REQUEST_BOUND');
ok(preflight.required_preflight_sequence?.length === 4, 'PREFLIGHT_SEQUENCE_LENGTH');
...
ok(preflight.pass_condition?.worker_mutation_count === 0, 'PREFLIGHT_ZERO_MUTATION');
...
ok(resolvedAssets === path.resolve(PORTAL_PATH), 'CONFIG_ASSET_RESOLUTION');
