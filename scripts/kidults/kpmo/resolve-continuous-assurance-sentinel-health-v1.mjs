...
import {readStableSentinelSnapshot} from './read-stable-sentinel-snapshot-v1.mjs';
...
async function collectLiveInputOnce(){
...
// A read-consistency retry never reuses a previous generation or receipt.
async function liveInput(){
  return readStableSentinelSnapshot(collectLiveInputOnce, {
    onRetry: observation => console.error(JSON.stringify(observation)),
  });
}
