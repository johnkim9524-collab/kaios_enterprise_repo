...
export function materialSummary(registry) {
  ...
  return {
    count: sorted.length,
    digest: materialRegistryDigest(sorted),
    members: sorted.map(item => item.issue_number),
    digest_scope: 'MATERIAL_DEFECT_REGISTRY_V3_STABLE_FIELDS'
  };
}
...
export async function buildLiveMaterialRegistry({repository, token}) {
  ...
  const registry=buildMaterialRegistry(openIssues);
...
function selfTest() {
  ...
  const registry = buildMaterialRegistry([
    {number:10,state:'open',title:'[P0] A',labels:[{name:'P0'}]},
    {number:11,state:'open',title:'P1: B',labels:[{name:'P1'}]}
  ]);
  ...
  const changedSummary = materialSummary(buildMaterialRegistry([
    {number:10,state:'open',title:'[P0] A',labels:[{name:'P0'}]},
    {number:11,state:'open',title:'P1: B',labels:[{name:'P1'}]},
    {number:12,state:'open',title:'[P1] C',labels:[{name:'P1'}]}
  ]));
  ...
  process.stdout.write(`${JSON.stringify({test:'CANONICAL_MATERIAL_REGISTRY_BINDING_V2',state:'VERIFIED_PASS',canonical_issue_count:canonicalIssues.length,canonical_parser_shared:true,negative_cases:mutations.length,new_defect_invalidates:true,severity_parity_required:true,pagination_duplicate_issue_rejected:true})}\n`);
}
...
export async function runLive() {
  ...
  try {
    ...
    const result={validator:'CANONICAL_MATERIAL_REGISTRY_BINDING_V2',state:failures.length?'VERIFIED_FAIL':'VERIFIED_PASS',protected_main_sha:mainSha,canonical_issue_count:canonicalIssues.length,material_defect_count:summary.count,material_defect_registry_binding_sha256:summary.digest,material_defect_registry_digest_scope:summary.digest_scope,material_defect_registry_members:summary.members,complete_open_issue_pagination:true,severity_parity_verified:true,canonical_parser_shared:true,failures,promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'};
    (failures.length?console.error:console.log)(JSON.stringify(result,null,2));
    if(failures.length) process.exitCode=1;
  } catch(error) {
    console.error(JSON.stringify({validator:'CANONICAL_MATERIAL_REGISTRY_BINDING_V2',state:'VERIFIED_FAIL',message:error instanceof Error?error.message:String(error),promotion_eligible:false,production:'HOLD',public:'HOLD',g5:'HOLD'},null,2));
    process.exitCode=1;
  }
}
