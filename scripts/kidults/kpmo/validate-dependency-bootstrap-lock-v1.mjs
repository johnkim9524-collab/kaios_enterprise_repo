...
export function validateDependencyBootstrapEstate(snapshot) {
  ...
  requireControl(contract.version === '1.0.0', 'CONTRACT_VERSION_DRIFT');
  ...
  for (const [workflowPath, raw] of Object.entries(snapshot.workflows)) {
    ...
    const nodeVersions = [...text.matchAll(/node-version:\s*['"]?([^'"\s}\]]+)/g)].map((match) => match[1]);
    const pythonVersions = [...text.matchAll(/python-version:\s*['"]?([^'"\s}\]]+)/g)].map((match) => match[1]);
...
if (process.argv[1]?.endsWith('validate-dependency-bootstrap-lock-v1.mjs')) {
  try {
    ...
    console.log(JSON.stringify({
      ...
      scope: 'GOVERNED_DEPENDENCY_BOOTSTRAP_PATHS',
      ...metrics,
