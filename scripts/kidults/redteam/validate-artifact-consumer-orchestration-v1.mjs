...
const runHistory = read(files.runHistory);
...
assert(!requirement.includes('--paginate') && requirement.includes('for PAGE in $(seq 1 25); do') && requirement.includes('-f branch=main -f head_sha="$SOURCE_SHA" -f event=workflow_run -f status=success') && requirement.includes('-f per_page=100 -f page="$PAGE"') && requirement.includes('PRIOR_SUCCESS_HISTORY_BUDGET_EXCEEDED') && requirement.includes('PRIOR_SUCCESS_PAGINATION_INCOMPLETE') && requirement.includes('--mode coverage-prior-success') && runHistory.includes('COVERAGE_PRIOR_SUCCESS_TITLE_FILTER_DRIFT') && runHistory.includes('pagination_required_for_semantic_exclusion'), 'REQUIREMENT_PRIOR_SUCCESS_SOURCE_TITLE_QUERY_MISSING');
...
const p0bExtractions = [...p1.matchAll(/unzip -q -o \/tmp\/p0b\.zip/g)].map((match) => match.index);
assert(p0bDigestGuards.length === 2 && p0bExtractions.length === 2 && p0bDigestGuards.every((index, position) => index < p0bExtractions[position]), 'P1_P0B_SAFE_ZIP_PRE_EXTRACTION_MISSING');
assert((p1.match(/--required-basename p0b-bounded-discovery-manifest-v1\.json/g) || []).length === 2, 'P1_P0B_REQUIRED_FILE_CARDINALITY_MISSING');
...
const supersessionBridgeEventMutation = supersession.replace('"${run_event}" == "workflow_dispatch"', '"${run_event}" == "push"');
assert(supersessionBridgeEventMutation !== supersession && !supersessionBridgeEventMutation.includes('"${run_event}" == "workflow_dispatch"'), 'EXACT_HEAD_SUPERSESSION_BRIDGE_EVENT_MUTATION_NOT_DETECTED');
...
console.log(JSON.stringify({
  ...
  adversarial_mutations_rejected: 36,
  production: 'HOLD',
  public_release: 'HOLD',
  g5: 'HOLD',
}, null, 2));