...
function normalizeKnownLegacyDesignerProvenance(fragment){
  const cloned=structuredClone(fragment);if(cloned?.id!=='kidults-er-designer-review-packet-r1'||!Array.isArray(cloned?.cases)||cloned.cases.length!==120)fail('DESIGNER_ACCEPTED_FRAGMENT_BOUNDARY');
  let affected=0,removed=0;
  for(const c of cloned.cases){
    const refs=c?.provenance_refs;if(!Array.isArray(refs)||!refs.length)continue;
    const unique=[...new Set(refs)];if(unique.length===refs.length)continue;
    const accepted=/^designer-r1-\d{3}$/.test(String(c.case_id||''));
    if(!accepted||c.case_class!=='SAME_OBJECT_NORMALIZATION'||c.rights_state!=='ALLOW'||refs.length!==4||unique.length!==3||refs.some(x=>typeof x!=='string'||!x.trim())||!c.source_a_reference||c.source_a_reference!==c.source_b_reference||!SHA.test(String(c.source_a_payload_sha256||''))||c.source_a_payload_sha256!==c.source_b_payload_sha256)fail('DESIGNER_PROVENANCE_DUPLICATE_OUTSIDE_ACCEPTED_SINGLE_RECORD_BOUNDARY',c?.case_id||'UNKNOWN');
    c.provenance_refs=unique;affected++;removed+=refs.length-unique.length;
  }
  if(affected!==40||removed!==40)fail('DESIGNER_PROVENANCE_DUPLICATE_COUNT_DRIFT',`${affected}/${removed}!=40/40`);
  return {fragment:cloned,affected_case_count:affected,removed_exact_duplicate_ref_occurrences:removed};
}
...
function adaptProvenance(rawCases){
  if(!Array.isArray(rawCases)||rawCases.length!==120)fail('PROVENANCE_EXACT_120_REQUIRED',rawCases?.length);
  const out=[];let multiObjectCases=0,totalBoundObjects=0,secondaryBoundObjects=0;
  for(const raw of rawCases){
    noResultLeak(raw);if(raw.stratum_id!=='er-stratum-provenance-unique-object')fail('PROVENANCE_STRATUM_MISMATCH',raw?.case_id);
    if(raw.case_class!=='AMBIGUOUS_REVIEW_REQUIRED'){out.push(basicCase(raw));continue;}
    const caseId=String(raw.case_id||'').trim(),a=String(raw.source_a_reference||'').trim(),da=String(raw.source_a_payload_sha256||'').trim();
    if(!caseId||raw.identity_boundary!=='MARKET_EVENT'||raw.rights_state!=='ALLOW'||!a||!SHA.test(da))fail('PROVENANCE_AMBIGUOUS_A_BOUNDARY',caseId||'UNKNOWN');
    const refs=Array.isArray(raw.source_b_references)?raw.source_b_references.map(String).sort():[];
    if(refs.length<2||new Set(refs).size!==refs.length||refs.some(x=>!x.trim()))fail('PROVENANCE_AMBIGUOUS_B_REFS_INVALID',caseId);
    const digestRows=Array.isArray(raw.source_b_payload_sha256)?raw.source_b_payload_sha256:[];
    if(digestRows.length!==refs.length)fail('PROVENANCE_AMBIGUOUS_B_DIGEST_COUNT',caseId);
    const byRef=new Map();for(const row of digestRows){const ref=String(row?.reference||'').trim(),digest=String(row?.payload_sha256||'').trim();if(!ref||!SHA.test(digest)||byRef.has(ref))fail('PROVENANCE_AMBIGUOUS_B_DIGEST_INVALID',caseId);byRef.set(ref,digest)}
    if(refs.some(ref=>!byRef.has(ref))||[...byRef.keys()].some(ref=>!refs.includes(ref)))fail('PROVENANCE_AMBIGUOUS_B_BINDING_MISMATCH',caseId);
    const primary=refs[0],primaryDigest=byRef.get(primary);
    const rawProv=strings(raw.provenance_refs,'PROVENANCE_REFS');if(!rawProv.includes(a)||refs.some(ref=>!rawProv.includes(ref)))fail('PROVENANCE_AMBIGUOUS_PROVENANCE_COVERAGE',caseId);
    const bindingRefs=refs.map(ref=>`${ref}#kidults-payload-sha256=${byRef.get(ref).slice(7)}`);
    out.push({case_id:caseId,stratum_id:raw.stratum_id,case_class:raw.case_class,identity_boundary:raw.identity_boundary,source_a_reference:a,source_b_reference:primary,source_a_payload_sha256:da,source_b_payload_sha256:primaryDigest,license_evidence_refs:strings(raw.license_evidence_refs,'LICENSE_REFS'),rights_state:'ALLOW',provenance_refs:[...new Set([...rawProv,...bindingRefs])].sort()});
    multiObjectCases++;totalBoundObjects+=refs.length;secondaryBoundObjects+=refs.length-1;
  }
  const ids=new Set();for(const c of out){if(ids.has(c.case_id))fail('PROVENANCE_DUPLICATE_CASE_ID',c.case_id);ids.add(c.case_id)}
  const count=field=>Object.fromEntries([...new Set(out.map(x=>x[field]))].sort().map(k=>[k,out.filter(x=>x[field]===k).length]));
  const cc=count('case_class'),ib=count('identity_boundary');if(JSON.stringify(cc)!==JSON.stringify({AMBIGUOUS_REVIEW_REQUIRED:50,HARD_NEGATIVE:20,TRANSACTION_TO_OBJECT_LINKAGE:50}))fail('PROVENANCE_CLASS_QUOTA',JSON.stringify(cc));if(JSON.stringify(ib)!==JSON.stringify({MARKET_EVENT:100,PHYSICAL_OBJECT:20}))fail('PROVENANCE_BOUNDARY_QUOTA',JSON.stringify(ib));
  if(multiObjectCases!==50||secondaryBoundObjects<50)fail('PROVENANCE_MULTI_OBJECT_NORMALIZATION_COUNT',`${multiObjectCases}/${secondaryBoundObjects}`);
  return {id:'kidults-er-provenance-accepted-lineage-adapter-r1',version:'1.0.0',status:'EXACT_120_ACCEPTED_LINEAGE_NORMALIZED_UNLABELED_NOT_REVIEWED',case_count:120,case_class_counts:cc,identity_boundary_counts:ib,multi_object_case_count:multiObjectCases,total_bound_object_count:totalBoundObjects,secondary_bound_object_count:secondaryBoundObjects,primary_object_rule:'LEXICOGRAPHIC_FIRST_SOURCE_B_REFERENCE',secondary_evidence_binding_rule:'ORIGINAL_SOURCE_REF_PLUS_FRAGMENT_SHA256_BINDING_TOKEN_NO_PAYLOAD',evidence_created:false,labels_present:false,empirical_pass:false,public_release:'HOLD',production:'HOLD',cases:out};
}
...
const designerNormalized=normalizeKnownLegacyDesignerProvenance(designer);
...
const provenanceAdapted=adaptProvenance(provenanceCases),provenance=packet(provenanceAdapted.cases);
const packets=[packet(casesOf(designerNormalized.fragment,'er-stratum-designer-maker-edition')),packet(pressing.cases),provenance,packet(serialized.cases),packet(variant.cases),packet(casesOf(vehicle,'er-stratum-vehicle-mechanical-asset'))];
...
const receipt={id:'kidults-er-base720-real-lineage-r2-receipt',version:'2.0.0',parent_issue:838,status:'PASS_EXACT_REAL_ACCEPTED_BASE720_MATERIALIZED_UNLABELED_NOT_REVIEWED',dataset_id:dataset.id,dataset_sha256:dataset.dataset_sha256,case_set_sha256:dataset.case_set_sha256,case_count:720,stratum_count:6,graded_population_case_count:0,validation,legacy_provenance_duplicate_normalization:{scope:'ONLY_LOCKED_ACCEPTED_EXACT_DUPLICATE_PROVENANCE_REFS',designer_single_record_normalization:{affected_case_count:designerNormalized.affected_case_count,removed_exact_duplicate_ref_occurrences:designerNormalized.removed_exact_duplicate_ref_occurrences},musicbrainz_expanded_hardnegative:{affected_case_count:residual1Normalized.affected_case_count+residual2Normalized.affected_case_count,removed_exact_duplicate_ref_occurrences:residual1Normalized.removed_exact_duplicate_ref_occurrences+residual2Normalized.removed_exact_duplicate_ref_occurrences},evidence_created:false},provenance_adapter:{case_count:provenanceAdapted.case_count,multi_object_case_count:provenanceAdapted.multi_object_case_count,total_bound_object_count:provenanceAdapted.total_bound_object_count,secondary_bound_object_count:provenanceAdapted.secondary_bound_object_count,primary_object_rule:provenanceAdapted.primary_object_rule,secondary_evidence_binding_rule:provenanceAdapted.secondary_evidence_binding_rule,evidence_created:false},release_adapter_receipts:{pressing:pressing.boundary_repair,variant:variant.boundary_repair},serialized_adapter:{case_count:serialized.case_count,wikidata_live_payload_revalidated:serialized.legacy_wikidata_live_payload_revalidated,nasm_exact_record_digest_count:serialized.legacy_nasm_source_record_digest_promoted_from_exact_record_sha256},reviewers:'NOT_BOUND_TO_PRE_REVIEW_PACKET',labels:'NOT_COLLECTED',empirical_pass:false,track_b:'NOT_STARTED',public_release:'HOLD',production:'HOLD'};
