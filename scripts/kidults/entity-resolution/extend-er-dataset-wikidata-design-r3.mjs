...
function claimItemIds(entity,pid){
  return (entity?.claims?.[pid]??[]).map(c=>c?.mainsnak?.datavalue?.value?.id).filter(Boolean);
}
...
const designCase={
  ...
  scope_id:'diagnostic-manufactured-vehicle-design-not-final-poc',
  expected:'MATCH',
  blind_holdout:false,
  ...
  design_label:label(design),
  claim_ceiling:'CANONICAL_DESIGN_IDENTITY_DIAGNOSTIC_ONLY'
};
if(dataset.cases.some(x=>x.case_id===designCase.case_id)) throw new Error('DUPLICATE_DESIGN_CASE');
const out={
  ...dataset,
  id:'entity-resolution-real-world-dataset-increment-r3',
  dataset_scope:'INCREMENTAL_PARTIAL_R3',
  scope_stratification_status:'INCOMPLETE',
  approved_scope_ids:[],
  required_scope_ids:[],
  source_families:[...new Set([...(dataset.source_families??[]),'wikidata-structured-data'])],
  cases:[...dataset.cases,designCase],
  truth_boundary:'R3 adds real CC0 CANONICAL_DESIGN / SAME_DESIGN_DIFFERENT_OBJECT evidence. This diagnostic manufactured-vehicle scope is not final approved PoC scope stratification and must not enable promotion.'
};
