...
if (ready.length !== 5 || ready.reduce((n,x)=>n+x.target_cases,0) !== 600) throw new Error('START_READY_600_REQUIRED');
if (blocked.reduce((n,x)=>n+x.target_cases,0) !== 240) throw new Error('BLOCKED_CONDITIONAL_240_REQUIRED');
const pressing = plan.lanes.find(x=>x.stratum_id==='er-stratum-pressing-edition-media');
if (!pressing || !pressing.source_families.includes('musicbrainz-core-database-dump-cc0')) throw new Error('MUSICBRAINZ_CC0_CORE_BOUNDARY_REQUIRED');
const variant=plan.lanes.find(x=>x.stratum_id==='er-stratum-variant-release-heavy');
if (!variant || variant.state!=='START_READY' || !variant.source_families.includes('wikidata-cc0-structured-product-identifiers') || !variant.rule.includes('live capacity gate')) throw new Error('WIKIDATA_VARIANT_CAPACITY_GATE_REQUIRED');
...
if (plan.parallel_execution?.start_ready_case_capacity !== 600 || plan.parallel_execution?.blocked_or_conditional_case_capacity !== 240) throw new Error('CAPACITY_TRUTH_MISMATCH');
...
console.log('PASS: Wave 1 has 600 start-ready cases across five rights-bounded lanes and fails closed on vehicle + graded 240 cases; reviewer/attestation/Track B remain unclaimed.');
