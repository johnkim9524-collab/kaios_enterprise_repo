...
for(const c of x.cases||[]){
 if(c.case_class!=='CROSS_MARKET_ALIAS'||c.identity_boundary!=='SOURCE_RECORD'||c.rights_state!=='ALLOW') throw new Error('CASE_SEMANTICS_INVALID');
 ...
 if(!Array.isArray(c.license_evidence_refs)||c.license_evidence_refs.length<2||!Array.isArray(c.provenance_refs)||c.provenance_refs.length<3) throw new Error('RIGHTS_OR_PROVENANCE_INVALID');
