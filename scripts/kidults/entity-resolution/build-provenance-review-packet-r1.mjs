...
if(corpus.status!=='REAL_SOURCE_LINKED_EVENT_OBJECT_CORPUS_UNLABELED') throw new Error('CORPUS_STATUS_INVALID');
if(corpus.stratum_id!=='er-stratum-provenance-unique-object') throw new Error('WRONG_STRATUM');
if(corpus.pair_count!==240||corpus.pairs?.length!==240) throw new Error('CORPUS_240_REQUIRED');
...
const cases=selected.map((p,index)=>({
  ...
  source_a_reference:p.event_source_reference,
  source_b_reference:p.object_source_reference,
  source_a_payload_sha256:p.event_payload_sha256,
  source_b_payload_sha256:p.object_payload_sha256,
  license_evidence_refs:p.license_evidence_refs,
  rights_state:p.rights_state,
  provenance_refs:p.provenance_refs,
  reviewer_prompt_context:{linkage_path:p.linkage_path,event_label:p.event_payload?.source_label||null,object_label:p.object_payload?.source_label||null},
  label:null,
