...
const maxShards=12,maxRecords=30000,maxCompressedBytes=35*1024*1024,timeoutMs=60000;
...
const shardUrls=indexText.split(/\r?\n/).map(s=>s.trim()).filter(Boolean).map(s=>/^https:\/\//i.test(s)?s:new URL(s,indexUrl).toString()).filter(u=>allowed(u)&&/\.(?:json|jsonl|ndjson|gz)(?:$|\?)/i.test(u)).sort().slice(0,maxShards);
