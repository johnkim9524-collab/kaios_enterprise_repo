...
const p=process.argv[2]||'/tmp/pressing-partial-review-packet-r1.json';const x=JSON.parse(await fs.readFile(p,'utf8'));
...
const hard=Number(x.case_class_counts?.HARD_NEGATIVE);if(!Number.isInteger(hard)||hard<0||hard>40)throw new Error('HARD_CAPACITY_INVALID');const expected=40+hard;
...
if(x.remaining_case_class_deficit?.HARD_NEGATIVE!==40-hard||x.remaining_case_class_deficit?.CROSS_MARKET_ALIAS!==40)throw new Error('DEFICITS');
if(x.source_record_reuse_across_cases!==0||x.labels_present!==false||x.model_predictions_present!==false||x.reviewer_a!=='NOT_ASSIGNED'||x.reviewer_b!=='NOT_ASSIGNED')throw new Error('REVIEW_BOUNDARY');
