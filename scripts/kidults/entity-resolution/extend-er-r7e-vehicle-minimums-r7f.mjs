...
if (!inputPath || !manifestPath) {
  throw new Error('Usage: node extend-er-r7e-vehicle-minimums-r7f.mjs <r7e.json> <manifest.json> [r7f.json]');
}

const dataset = JSON.parse(await fs.readFile(inputPath, 'utf8'));
const manifest = JSON.parse(await fs.readFile(manifestPath, 'utf8'));
const STRATUM = 'er-stratum-vehicle-mechanical-asset';
const MODEL = 'Q1002954'; // Formula One car class used only for bounded constructed-control mechanics.
...
// Exclude every physical object already used by serialized-reference controls so
// the vehicle grammar adds distinct source records rather than duplicated scored pairs.
const used = new Set();
for (const item of dataset.cases) {
  if (item.scope_id === 'er-stratum-serialized-reference') {
    for (const side of [item.left, item.right]) {
      if (side?.unique_keys?.object_id) used.add(String(side.unique_keys.object_id));
    }
  }
}
...
for (const row of queryResult?.results?.bindings ?? []) {
  ...
  if (qid && serial && !used.has(qid)) rawCandidates.push({qid, serial});
}
const candidates = [...new Map(rawCandidates
  .sort((left, right) => compareText([left.qid, left.serial].join('\0'), [right.qid, right.serial].join('\0')))
  .map((item) => [item.qid, item])).values()];
if (candidates.length < 2) throw new Error(`INSUFFICIENT_DISTINCT_SERIALIZED_VEHICLE_SOURCE_RECORDS:${candidates.length}`);
let selected = null;
outer: for (let leftIndex = 0; leftIndex < candidates.length; leftIndex += 1) {
  for (let rightIndex = leftIndex + 1; rightIndex < candidates.length; rightIndex += 1) {
    if (candidates[leftIndex].serial !== candidates[rightIndex].serial) {
      selected = {left:candidates[leftIndex], right:candidates[rightIndex]};
      break outer;
    }
  }
}
...
if (!items(left, 'P31').includes(MODEL) || !items(right, 'P31').includes(MODEL)) {
  throw new Error('VEHICLE_MODEL_SOURCE_PAYLOAD_REVALIDATION_FAILED');
}
if (!strings(left, 'P2598').includes(selected.left.serial) || !strings(right, 'P2598').includes(selected.right.serial)) {
  throw new Error('VEHICLE_SERIAL_SOURCE_PAYLOAD_REVALIDATION_FAILED');
}

const sameAnchor = `wikidata-vehicle-record:${selected.left.qid}:${selected.left.serial}`;
const normalization = {
  case_id:`wikidata-vehicle-normalization-${selected.left.qid}`,
  ...
  left:{
    anchors:{SOURCE_RECORD:sameAnchor}, unique_keys:{object_id:selected.left.qid, serial:selected.left.serial},
    entity_id:selected.left.qid, model_id:MODEL, vehicle_class:label(model), source:'wikidata-structured-data',
  },
  right:{
    anchors:{SOURCE_RECORD:sameAnchor}, unique_keys:{object_id:selected.left.qid, serial:selected.left.serial},
    entity_id:selected.left.qid, model_id:MODEL, vehicle_class:label(model), source:'wikidata-structured-data',
  ...
  provenance_refs:[
    `wikidata:${selected.left.qid}:P31:${MODEL}`,
    `wikidata:${selected.left.qid}:P2598:${selected.left.serial}`,
    'wikidata:Q1002954:Formula-One-car',
  ],
  rights_state:'ALLOW',
  label_basis:'ALGORITHMICALLY_CONSTRUCTED_FROM_WIKIDATA_ENTITYDATA_FOR_ONE_SERIALIZED_FORMULA_ONE_CAR_SOURCE_ITEM',
  source_evidence:[
    sourceEvidence(selected.left.qid, leftPayload),
    sourceEvidence(MODEL, modelPayload),
  ],
  claim_ceiling:'VEHICLE_MECHANICAL_SOURCE_RECORD_IDENTITY_ONLY_NO_MARKET_EVIDENCE',
};
const hardNegative = {
  case_id:`wikidata-vehicle-hard-negative-${selected.left.qid}-${selected.right.qid}`,
  ...
  label_review_status:'NOT_INDEPENDENTLY_REVIEWED_OR_ADJUDICATED',
  left:{
    anchors:{PHYSICAL_OBJECT:`wikidata-vehicle:${selected.left.qid}:${selected.left.serial}`},
    unique_keys:{object_id:selected.left.qid, serial:selected.left.serial}, entity_id:selected.left.qid,
    model_id:MODEL, source:'wikidata-structured-data',
  },
  right:{
    anchors:{PHYSICAL_OBJECT:`wikidata-vehicle:${selected.right.qid}:${selected.right.serial}`},
    unique_keys:{object_id:selected.right.qid, serial:selected.right.serial}, entity_id:selected.right.qid,
    model_id:MODEL, source:'wikidata-structured-data',
  },
  provenance_refs:[
    `wikidata:${selected.left.qid}:P2598:${selected.left.serial}`,
    `wikidata:${selected.right.qid}:P2598:${selected.right.serial}`,
    `wikidata:${selected.left.qid}:P31:${MODEL}`,
    `wikidata:${selected.right.qid}:P31:${MODEL}`,
  ],
  rights_state:'ALLOW',
  label_basis:'ALGORITHMICALLY_CONSTRUCTED_FROM_WIKIDATA_ENTITYDATA_FOR_TWO_DISTINCT_FORMULA_ONE_CAR_SOURCE_ITEMS_WITH_DISTINCT_SERIALS',
  source_evidence:[
    sourceEvidence(selected.left.qid, leftPayload),
    sourceEvidence(selected.right.qid, rightPayload),
    sourceEvidence(MODEL, modelPayload),
  ],
  claim_ceiling:'VEHICLE_MECHANICAL_PHYSICAL_OBJECT_IDENTITY_ONLY_NO_MARKET_EVIDENCE',
};
for (const item of [normalization, hardNegative]) {
  if (dataset.cases.some((existing) => existing.case_id === item.case_id)) {
    throw new Error(`DUPLICATE_R7F_CASE:${item.case_id}`);
  }
}
...
const out = {
  ...
  truth_boundary:'R7F adds two live-source-derived, algorithmically labeled VEHICLE_MECHANICAL_ASSET constructed controls from distinct serialized source items. They are not independently reviewed, adjudicated, or blind. Together with the existing canonical-design control, this may complete only the declared vehicle per-stratum mechanics; empirical accuracy, global strata completion, current-market claims, Track B, and Production remain blocked.',
};
...
console.log(JSON.stringify({
  ...
  excluded_prior_objects:sortedUnique([...used]),
  selected,
