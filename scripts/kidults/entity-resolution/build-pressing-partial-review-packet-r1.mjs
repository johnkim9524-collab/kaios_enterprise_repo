...
if(capacity.status!=='COMPLETE_FAIL_CLOSED_PARTIAL_CAPACITY'||capacity.metrics?.conservative_case_capacity!==45) throw new Error('CAPACITY_45_REQUIRED');
if(capacity.metrics?.same_object_normalization_capacity!==40||capacity.metrics?.hard_negative_capacity!==5||capacity.metrics?.cross_market_alias_capacity!==0) throw new Error('CLASS_CAPACITY_INVALID');
...
const hard=(capacity.candidate_pools?.HARD_NEGATIVE||[]).slice(0,5);
...
if(hard.length!==5||normal.length!==40) throw new Error(`SOURCE_DISJOINT_SELECTION_${normal.length}_NORMAL_${hard.length}_HARD`);
...
if(cases.length!==45||new Set(selected.flatMap(c=>c.source_record_ids||[])).size!==selected.flatMap(c=>c.source_record_ids||[]).length) throw new Error('CASE_OR_SOURCE_DISJOINTNESS_INVALID');
if(cases.some(c=>c.rights_state!=='ALLOW')) throw new Error('RIGHTS_NOT_ALLOW');
const blindIds=cases.filter((_,i)=>i%2===0).slice(0,22).map(c=>c.case_id);
const packet={
  id:'kidults-er-pressing-partial-review-packet-r1',version:'1.0.0',status:'PARTIAL_REVIEW_PACKET_READY_UNLABELED_NOT_REVIEWED',stratum_id:corpus.stratum_id,
  case_count:45,sampling_target_total:120,remaining_case_count:75,
  case_class_counts:{SAME_OBJECT_NORMALIZATION:40,HARD_NEGATIVE:5,CROSS_MARKET_ALIAS:0},
  remaining_case_class_deficit:{SAME_OBJECT_NORMALIZATION:0,HARD_NEGATIVE:35,CROSS_MARKET_ALIAS:40},
  identity_boundary_counts:{SOURCE_RECORD:40,PHYSICAL_OBJECT:5},
  ...
  blind_partition:{state:'CANDIDATE_NOT_SEALED',case_count:22,case_ids:blindIds,partition_sha256:sha(JSON.stringify(blindIds))},cases,
  empirical_pass:false,track_b:'NOT_STARTED',public_release:'HOLD',production:'HOLD',
  truth_boundary:'45 real MusicBrainz source-bound Pressing/Edition cases are packetized: 40 SAME_OBJECT_NORMALIZATION and 5 HARD_NEGATIVE. The remaining 35 hard-negative and 40 cross-market-alias cases are not fabricated. No reviewers, labels, sealed holdout, empirical PASS, Track B, publication or Production are claimed.'
};
await fs.writeFile(outPath,JSON.stringify(packet,null,2));
console.log(JSON.stringify({status:packet.status,case_count:45,remaining_case_count:75,class_counts:packet.case_class_counts,blind_candidate_count:22,production:'HOLD'},null,2));
