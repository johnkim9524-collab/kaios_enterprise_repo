...
if (portalK100) {
  if (portalK100.asset_standard?.id !== "K100_EDITORIAL_ASSET_V1") errors.push("Portal K100 asset standard is not registered.");
  if (portalK100.asset_standard?.unverified_visual_policy !== "WITHHOLD") errors.push("Portal K100 must withhold unverified visuals.");
  for (const item of portalK100.items ?? []) {
    if (item.visual_role !== "EDITORIAL_INTERPRETATION") errors.push(`${item.id}: visual role must be EDITORIAL_INTERPRETATION.`);
    if (!Number.isFinite(Number(item.display_scale)) || Number(item.display_scale) <= 0) errors.push(`${item.id}: display_scale must be a positive number.`);
  }
}
...
if (toys?.visual_status !== "VISUAL_WITHHELD_PENDING_EVIDENCE") errors.push("Toys & Models must declare VISUAL_WITHHELD_PENDING_EVIDENCE.");
if (manifest?.display_policy?.featured_slice_count !== 4) errors.push("Manifest featured_slice_count must be 4.");
if (manifest?.display_policy?.unverified_visual_policy !== "WITHHOLD") errors.push("Manifest unverified_visual_policy must be WITHHOLD.");
