...
const market=[
 event('a','S1',100000,'USD','2026-07-01T00:00:00Z','V1','9'),
 event('b','S2',90000,'EUR','2026-07-15T00:00:00Z','V2','8'),
 event('c','S1',105000,'USD','2026-08-01T00:00:00Z','V3','9')
];
market[2].event.event_state='RELISTED';
market[2].event.evidence_class='VERIFIED_SOLD_EVENT';
market[2].event.relist_parent_event_id='a';

const normalized=normalizeMarketCurrencies(market,fx,{targetCurrency:'USD',asOf:now});
assert.equal(normalized.filter(x=>x.fx_state==='PASS').length,3);
assert.equal(normalized[1].event.price.normalized_amount,108000);
assert.equal(selectComparables(normalized,{canonical_entity_id:'vehicle:1',region:'NA'}).length,2);

const policy={id:'grade-policy-test',max_adjustment_ratio:0.5,factors:{'8':0.8,'9':0.9,'10':1}};
assert.equal(adjustForCondition(80000,'8','9',policy).amount,90000);
assert.equal(adjustForCondition(80000,null,'9',policy).state,'NO_ADJUSTMENT_CLAIM');
assert.equal(adjustForCondition(80000,'8','10',{...policy,max_adjustment_ratio:0.1}).state,'HOLD');

const relist=analyzeRelistings(market);
assert.equal(relist.relisted_event_count,1);
assert.ok(relist.relisting_rate>0);
assert.equal(classifyIntelligenceConfidence({eventCount:2,sourceOwnerCount:2,venueCount:2}).classification,'NOT_VERIFIED');
assert.equal(classifyIntelligenceConfidence({eventCount:10,sourceOwnerCount:3,venueCount:3,dispersionRatio:0.1}).classification,'HIGH');
assert.equal(classifyIntelligenceConfidence({eventCount:10,sourceOwnerCount:3,venueCount:3,conflictCount:1}).classification,'HOLD_CONFLICT');

const grading=(provider,total)=>canonicalizeGradingEvidence({
 grading_evidence_id:`g-${provider}`,provider_id:provider,canonical_entity_id:'card:1',provider_item_id:`${provider}-1`,certification_number:`${provider}-cert`,
 identity:{year:2000,set:'S',card_number:'1',subject:'X',variant:'A',language:'EN'},grade:{raw_grade:'9',scale_id:'TEN',scale:{min:0,max:10,higherIsBetter:true}},
 population:{at_grade:10,higher:2,total,scope:'PROVIDER_CENSUS',as_of:now},observed_at:now,
 rights:{collect:'ALLOW',store:'ALLOW',transform:'ALLOW',internal_display:'ALLOW'},lineage:{source_owner:provider,source_record_ref:`${provider}:1`,adapter_version:'test'},admission:{confidence:0.9}
});
...
assert.equal(result.fx.normalized_count,3);
