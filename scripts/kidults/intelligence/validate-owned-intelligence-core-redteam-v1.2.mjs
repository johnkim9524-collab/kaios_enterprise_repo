...
import {canonicalizeMarketEvent,canonicalizeGradingEvidence,gradingAdmissionErrors,marketAdmissionErrors} from './provider-independent-layers-v1.mjs';
import {buildOwnedIntelligence,buildValuationIntelligence} from './owned-intelligence-core-v1.mjs';
import {enforceRightsLifecycle,convertCurrency,selectComparables,adjustForCondition,analyzeRelistings,classifyIntelligenceConfidence,buildOwnedIntelligenceV11} from './owned-intelligence-core-v1.1.mjs';
...
const fxFact=(owner,rate,observed_at='2026-08-20T00:00:00Z')=>({base_currency:'EUR',quote_currency:'USD',rate,observed_at,source_owner:owner,rights:{collect:'ALLOW',store:'ALLOW',transform:'ALLOW'}});
...
const neutral=convertCurrency(100,'EUR','USD',[fxFact('A',1.10,'2026-08-19T00:00:00Z'),fxFact('A',1.101,'2026-08-20T00:00:00Z'),fxFact('A',1.101,'2026-08-20T00:00:00Z'),fxFact('B',1.103)],{asOf:now});
assert.equal(neutral.source_owner_count,2);assert.ok(Math.abs(neutral.rate-1.102)<0.00001);
assert.equal(convertCurrency(100,'EUR','USD',[fxFact('A',1.1),fxFact('A',1.2)],{asOf:now}).reason,'FX_SAME_SOURCE_CONFLICT');

const badGrade=canonicalizeGradingEvidence({grading_evidence_id:'bad-g',provider_id:'G',canonical_entity_id:'card:bad',provider_item_id:'p',identity:{},grade:{raw_grade:'9',scale:{min:0,max:10,higherIsBetter:true}},population:{at_grade:80,higher:30,total:100,scope:'PROVIDER_CENSUS',as_of:now},observed_at:now,rights:{collect:'ALLOW',store:'ALLOW',transform:'ALLOW'},lineage:{source_owner:'G',source_record_ref:'r'},admission:{confidence:.9}});
assert.equal(badGrade.admission.state,'HOLD');assert.ok(gradingAdmissionErrors(badGrade).includes('POPULATION_AT_OR_ABOVE_EXCEEDS_TOTAL'));
const badSold={schema_version:'market-event-v1',market_event_id:'bad',evidence_class:'VERIFIED_SOLD_EVENT',event_state:'SOLD',canonical_entity_id:'asset:bad',venue_id:'V',event_at:now,price:{price_type:'HAMMER',amount:0,currency:'USD'},rights:{collect:'ALLOW',store:'ALLOW',transform:'ALLOW'},lineage:{source_family_id:'S',evidence_id:'E'}};
assert.ok(marketAdmissionErrors(badSold).includes('SOLD_PRICE_INCOMPLETE_OR_INVALID'));
...
const missingRegion=structuredClone(make('r1','S1'));delete missingRegion.event.region;assert.equal(selectComparables([missingRegion],{canonical_entity_id:'asset:1',region:'NA'}).length,0);
...
const dup=make('dup','S1');dup.event.relist_parent_event_id='parent';const rel=analyzeRelistings([dup,structuredClone(dup)]);assert.equal(rel.observed_event_count,1);assert.equal(rel.relisted_event_count,1);
...
assert.equal(buildOwnedIntelligenceV11({market:partialFx,fxFacts:[],targetCurrency:'USD',asOf:now}).status,'HOLD_FX_INCOMPLETE_OR_CONFLICT');
const policy={id:'p',version:'1.0.0',max_adjustment_ratio:.1,factors:{8:.8,9:.9,10:1}};
const condMarket=[make('q1','S1'),make('q2','S2','asset:1',101000,'USD','2026-08-05T00:00:00Z'),make('q3','S3','asset:1',102000,'USD','2026-08-10T00:00:00Z')];for(const x of condMarket)x.event.condition_grade.grade='8';
assert.equal(buildOwnedIntelligenceV11({market:condMarket,targetCurrency:'USD',asOf:now,targetGrade:'10',conditionPolicy:policy}).status,'HOLD_CONDITION_ADJUSTMENT_NOT_PASS');
assert.equal(buildOwnedIntelligenceV11({market:[make('v1','S1','asset:1'),make('v2','S2','asset:2')],targetCurrency:'USD',asOf:now}).status,'HOLD_MULTI_ENTITY_VALUATION_INPUT');

console.log(JSON.stringify({status:'PASS',controls:{rights_dates:'PASS',retention_domain:'PASS',fx_positive_only:'PASS',fx_conflict_quarantine:'PASS',fx_source_neutrality:'PASS',grading_population_sanity:'PASS',sold_price_sanity:'PASS',fx_subset_cherrypick_blocked:'PASS',comparable_dimensions:'PASS',versioned_condition_policy:'PASS',condition_hold_blocks_valuation:'PASS',relisting_dedupe:'PASS',confidence_domain:'PASS',multi_entity_core:'PASS',sold_source_owners_only:'PASS',future_date_hold:'PASS'}},null,2));
