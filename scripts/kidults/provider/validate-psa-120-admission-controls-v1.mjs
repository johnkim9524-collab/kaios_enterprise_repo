...
import { assertPsaKnownCertManifest, validatePsaKnownCertManifest } from './validate-psa-manifest-provenance-contract-v1.mjs';
...
const authorityRegistry = JSON.parse(fs.readFileSync('coordination/kidults/provider/psa-source-authority-registry-v1.json','utf8'));
...
if (retention.state !== 'IMPLEMENTED_VALIDATOR_CONNECTED_RUNTIME_ACTIVATION_HOLD'
  || !retention.implementation?.store_module
  || !retention.implementation?.deletion_runner
  || retention.implementation?.automatic_trigger !== 'BLOCKED_PENDING_PERSISTENT_GOVERNED_RUNNER_AND_PRIVATE_STORE_MOUNT'
  || retention.implementation?.github_hosted_runner_execution !== 'VALIDATION_ONLY_NO_PERSISTENT_STORE'
  || retention.implementation?.production_activation !== 'HOLD') {
  throw new Error('PSA_RETENTION_IMPLEMENTATION_OR_ACTIVATION_STATE_INVALID');
}
...
if (!executionPlan.manifest_progress
  || JSON.stringify(Object.keys(executionPlan.manifest_progress).sort()) !== JSON.stringify(['admission_counter','declared_known_count_is_not_admission_progress','remaining_counter','source'].sort())
  || executionPlan.state !== 'GOVERNED_EXECUTION_PLAN_STAGED'
  || executionPlan.manifest_progress?.source !== 'coordination/kidults/provider/psa-120-known-cert-manifest-v1.json'
  || executionPlan.manifest_progress?.admission_counter !== 'provenance_bound_admissible_count'
  || executionPlan.manifest_progress?.remaining_counter !== 'remaining_required'
  || executionPlan.manifest_progress?.declared_known_count_is_not_admission_progress !== true) throw new Error('PSA_EXECUTION_MANIFEST_BINDING_INVALID');
if (executionPlan.live_execution !== 'HOLD_UNTIL_PRECONDITIONS') throw new Error('PSA_LIVE_EXECUTION_MUST_HOLD');

const key = randomBytes(32);
const certReferenceKey = randomBytes(32);
const observedAt = new Date('2026-08-28T00:00:00.000Z');
const syntheticCertNumber = '0'.repeat(8);
const record = buildPrivatePsaRecord({ certNumber: syntheticCertNumber, certReferenceKey, payload: { PSACert: { Brand: 'TEST', TotalPopulation: 1 } }, key, observedAt });
...
if (!/^hmac-sha256:v1:[0-9a-f]{64}$/.test(record.cert_reference_digest)) throw new Error('PSA_PRIVATE_RECORD_CERT_REFERENCE_NOT_KEYED');
...
try { buildPrivatePsaRecord({ certNumber: syntheticCertNumber, certReferenceKey, payload: {}, key, observedAt }); } catch (e) { negativePass = e.message === 'PSA_CERT_PAYLOAD_REQUIRED'; }
if (!negativePass) throw new Error('PSA_PAYLOAD_SHAPE_NEGATIVE_TEST_FAILED');
negativePass = false;
try { buildPrivatePsaRecord({ certNumber: syntheticCertNumber, certReferenceKey, payload: { PSACert: { Brand: 'TEST', Price: 100 } }, key, observedAt }); } catch (e) { negativePass = e.message === 'PSA_PAYLOAD_FIELD_NOT_ALLOWED:Price'; }
if (!negativePass) throw new Error('PSA_UNAPPROVED_FIELD_NEGATIVE_TEST_FAILED');
negativePass = false;
try { buildPrivatePsaRecord({ certNumber: syntheticCertNumber, certReferenceKey, payload: { PSACert: { Brand: 'TEST' } }, key: Buffer.alloc(16), observedAt }); } catch (e) { negativePass = e.message === 'PSA_AES_256_KEY_REQUIRED'; }
if (!negativePass) throw new Error('PSA_WEAK_KEY_NEGATIVE_TEST_FAILED');
negativePass = false;
try { buildPrivatePsaRecord({ certNumber: syntheticCertNumber, payload: { PSACert: { Brand: 'TEST' } }, key, observedAt }); } catch (e) { negativePass = e.message === 'PSA_CERT_REFERENCE_KEY_REQUIRED'; }
if (!negativePass) throw new Error('PSA_REFERENCE_KEY_NEGATIVE_TEST_FAILED');
negativePass = false;
...
const fixture = {
  cert_reference_digest: `hmac-sha256:v1:${'a'.repeat(64)}`,
  source_authority_id: 'PSA_SOURCE_AUTHORITY_SYNTHETIC_VALIDATOR_V1',
  source_authority_entry_digest: `sha256:${'b'.repeat(64)}`,
  source_bundle_token: `hmac-sha256:v1:${'c'.repeat(64)}`,
  source_record_token: `hmac-sha256:v1:${'d'.repeat(64)}`,
  source_receipt_digest: `sha256:${'e'.repeat(64)}`,
...
for (const [expectedCode, mutate] of [
  ['PSA_MANIFEST_ADMISSIBLE_COUNT_MISMATCH', x => { x.provenance_bound_admissible_count = 1; }],
  ['PSA_MANIFEST_ENTRY_1_DUPLICATE_CERT_REFERENCE', x => { x.entries = [fixture, { ...fixture }]; x.provenance_bound_admissible_count = 2; x.remaining_required = 118; }],
  ['PSA_MANIFEST_ENTRY_0_SCHEMA_KEYS_INVALID', x => { x.entries = [{ ...fixture, cert_number: 'synthetic-control-value' }]; x.provenance_bound_admissible_count = 1; x.remaining_required = 119; }],
  ['PSA_MANIFEST_ENTRY_0_ENUMERATION_INVALID', x => { x.entries = [{ ...fixture, enumeration_used: true }]; x.provenance_bound_admissible_count = 1; x.remaining_required = 119; }]
]) {
  const x = structuredClone(manifest);
  mutate(x);
  const errors = validatePsaKnownCertManifest(x, { authorityRegistry });
  if (!errors.includes(expectedCode)) throw new Error(`PSA_MANIFEST_NEGATIVE_MUTATION_NOT_REJECTED:${expectedCode}:${errors.join(',')}`);
}
...
console.log(JSON.stringify({
  ...
  retention_runtime_activation: 'HOLD_PENDING_PERSISTENT_GOVERNED_RUNNER_AND_PRIVATE_STORE_MOUNT',
  ...
  duplicate_hmac_reference_rejected: true,
  governed_source_authority_registry_bound: true,
