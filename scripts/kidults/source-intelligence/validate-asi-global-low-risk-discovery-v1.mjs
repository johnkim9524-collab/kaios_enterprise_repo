...
for(const p of ['WIKIDATA_OFFICIAL_WEBSITE_GRAPH','OPENSTREETMAP_NOMINATIM_PUBLIC_METADATA'])if(!x.discovery_providers?.includes(p))fail(`provider lane missing ${p}`);
if(Number(x.candidate_count||0)<1)fail('no empirically discovered candidate');
if(!Object.values(x.provider_counts||{}).some(v=>Number(v)>0))fail('no healthy provider produced candidates');
...
console.log(JSON.stringify({status:'PASS',candidates:x.candidate_count,providers:x.provider_counts,regions:x.region_counts,request_errors:(x.request_errors||[]).length,production:x.production},null,2));
