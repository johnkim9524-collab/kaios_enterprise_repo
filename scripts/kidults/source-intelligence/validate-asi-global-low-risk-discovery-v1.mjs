...
if(x.primary_target!=='GLOBAL_ANY_SITE_SOURCE_UNIVERSE')fail('primary target must be global any-site source universe');
if(x.universe_boundary!=='ANY_PUBLICLY_DISCOVERABLE_SITE_OR_SOURCE_ENDPOINT')fail('universe boundary narrowed');
if(x.source_family_restriction!==null)fail('source family must not restrict discovery universe');
if(x.discovery_intents_are_recall_hints_only!==true)fail('discovery intents must be hints only');
...
for(const d of ['WIKIDATA_OFFICIAL_WEBSITE_GRAPH','OPENSTREETMAP_NOMINATIM_PUBLIC_METADATA'])if(!x.discovery_fabric?.includes(d))fail(`discovery fabric missing ${d}`);
if(Number(x.candidate_count||0)<1)fail('no empirically discovered candidate');
if(!Object.values(x.provider_counts||{}).some(v=>Number(v)>0))fail('no healthy discovery fabric produced candidates');
...
for(const c of x.candidates||[]){
  if(c.source_family_hint!=='UNCLASSIFIED_ANY_SITE_CANDIDATE')fail('candidate source family was pre-restricted');
  if(c.universe_target!=='GLOBAL_ANY_SITE_SOURCE_UNIVERSE')fail('candidate universe target mismatch');
  if(c.terminal_transaction_asserted!==false)fail('discovery cannot assert terminal transaction');
...
console.log(JSON.stringify({status:'PASS',target:x.primary_target,candidates:x.candidate_count,providers:x.provider_counts,regions:x.region_counts,request_errors:(x.request_errors||[]).length,production:x.production},null,2));
