...
if(x.status!=='SHADOW_GLOBAL_OPEN_MARKET_DISCOVERY_COMPLETE_NOT_RIGHTS_ADMITTED')fail('status mismatch');
if(x.primary_target!=='GLOBAL_OPEN_MARKET_UNIVERSE')fail('primary target must be global open market universe');
if(x.source_family!=='OPEN_MARKETPLACE_OR_DEALER')fail('source family mismatch');
...
for(const p of ['WIKIDATA_OFFICIAL_WEBSITE_GRAPH','OPENSTREETMAP_NOMINATIM_PUBLIC_METADATA'])if(!x.discovery_fabric?.includes(p))fail(`discovery fabric missing ${p}`);
if(Number(x.candidate_count||0)<1)fail('no empirically discovered open-market candidate');
...
for(const c of x.candidates||[]){
 if(c.source_family_hint!=='OPEN_MARKETPLACE_OR_DEALER'||c.market_target_priority!=='PRIMARY_GLOBAL_OPEN_MARKET')fail('candidate is not open-market-first');
