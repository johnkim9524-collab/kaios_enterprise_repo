...
if(Number(x.rotation_count)!==4||Number(x.scheduled_scope_count)!==32||!Array.isArray(x.scheduled_scope_ids)||new Set(x.scheduled_scope_ids).size!==32)fail('FULL_SCOPE_SCHEDULE');
if(!Number.isInteger(Number(x.empirical_scope_count))||Number(x.empirical_scope_count)<1||Number(x.empirical_scope_count)>32||!Array.isArray(x.empirical_scope_ids)||new Set(x.empirical_scope_ids).size!==Number(x.empirical_scope_count)||!x.empirical_scope_ids.every(s=>x.scheduled_scope_ids.includes(s)))fail('EMPIRICAL_SCOPE_COVERAGE');
...
const r=x.rules||{};for(const k of ['full_32_scope_schedule_attempted','empirical_coverage_reported_separately','multi_provider_fail_soft','discovery_metadata_only','attention_is_not_demand','listing_is_not_sold','terminal_transaction_assertion_required','reserve_is_not_safe_pool','rights_never_promoted','admission_never_promoted','target_site_body_traversal_forbidden','content_acquisition_forbidden'])if(r[k]!==true)fail(`RULE:${k}`);
const allowed=new Set(['OPENALEX_PUBLIC_WORK_AND_SOURCE_METADATA','GDELT_PUBLIC_DOMAIN_MENTION_METADATA']);const seen=new Set(),hosts=new Set(),empiricalScopes=new Set();
for(const c of x.candidates){
 ...
 const scopes=c.scope_hints||[c.scope_hint].filter(Boolean);if(!scopes.length||!scopes.every(s=>x.scheduled_scope_ids.includes(s)))fail('SCOPE_ESCAPE');for(const s of scopes)empiricalScopes.add(s);
...
if(hosts.size!==Number(x.unique_host_count))fail('HOST_RECOUNT');if(empiricalScopes.size!==Number(x.empirical_scope_count))fail('EMPIRICAL_SCOPE_RECOUNT');
const sum=Object.values(x.provider_counts||{}).reduce((a,b)=>a+Number(b||0),0);if(sum<Number(x.candidate_count))fail('PROVIDER_COUNTS');
console.log(JSON.stringify({status:'PASS',scheduled_scopes:x.scheduled_scope_count,empirical_scopes:x.empirical_scope_count,candidates:x.candidate_count,hosts:x.unique_host_count,healthy_lanes:x.healthy_lane_count,degraded_lanes:x.degraded_lane_count,production:'HOLD'}));
