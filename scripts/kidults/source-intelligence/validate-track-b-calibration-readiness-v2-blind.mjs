...
export function validateTrackBCalibrationReadinessBlind(directory) {
  ...
  if (review.version !== "2.3.0") fail(errors, "Blind review package must be version 2.3.0.");
  ...
  if (review.blindness_hardening?.channel_bonus_exposed !== false) fail(errors, "Channel bonus must not be exposed.");
  ...
  for (const record of review.records) {
    ...
    if (record.best_source_role_evidence && Object.prototype.hasOwnProperty.call(record.best_source_role_evidence, "channel_bonus")) fail(errors, `${record.review_case_id}: Source-role channel bonus leaked.`);
  ...
  if (manifest.automated_match_scores_exposed !== false || manifest.token_weights_exposed !== false || manifest.channel_bonus_exposed !== false) fail(errors, "Run manifest evidence-only flags are invalid.");
...
if (errors.length) {
  console.error(`KIDULTS Track B Calibration Readiness v2.3 Evidence-Only Blindness: FAIL (${errors.length})`);
  for (const error of errors) console.error(`ERROR: ${error}`);
  process.exit(1);
}
console.log("KIDULTS Track B Calibration Readiness v2.3 Evidence-Only Blindness: PASS");
console.log("Original bucket leakage: 0; automated match scores: 0; token weights: 0; channel bonus: 0; deterministic blind shuffle: PASS; Production: HOLD");
