...
const [runtimeContractPath,frontierPath,referenceContractPath,templateContractPath,rightsPreflightPath,outputDir] = process.argv.slice(2);
if (![runtimeContractPath,frontierPath,referenceContractPath,templateContractPath,rightsPreflightPath,outputDir].every(Boolean)) throw new Error('SOURCE_ADAPTER_TEMPLATE_ARGUMENTS_REQUIRED');
...
const [runtime,reference,contract,rightsPreflight,frontierText]=await Promise.all([
  readJson(runtimeContractPath),readJson(referenceContractPath),readJson(templateContractPath),readJson(rightsPreflightPath),fs.readFile(frontierPath,'utf8')
]);
...
const sourceIds=runtime.registered_source_profiles.map(([,sourceId])=>sourceId);
const rightsIndex=buildPurposeRightsIndex(rightsPreflight,sourceIds,'CURRENT_SOLD_TRANSACTION_AND_LIQUIDITY_ACQUISITION');
const profiles=runtime.registered_source_profiles.map(([priorityRank,sourceId,verifiedAssignmentCount,targetClaims])=>{
  ...
  const rights=rightsIndex.get(sourceId);
  return{
    ...
    rights_eligibility_state:rights.decision,
    rights_eligibility_reason_codes:rights.reason_codes,
    rights_evidence_refs:rights.evidence_refs,
    eligible_for_acquisition_or_adapter_backlog:rights.decision===RIGHTS_CLEAR,
...
const registry={
  ...
  rights_clear_profile_count:profiles.filter(p=>p.rights_eligibility_state===RIGHTS_CLEAR).length,
  rights_hold_profile_count:profiles.filter(p=>p.rights_eligibility_state!==RIGHTS_CLEAR).length,
  adapter_backlog_eligible_profile_count:profiles.filter(p=>p.eligible_for_acquisition_or_adapter_backlog).length,
...
const eligibleItems=profiles.filter((p)=>p.eligible_for_acquisition_or_adapter_backlog);
const rightsQueue=profiles.filter((p)=>!p.eligible_for_acquisition_or_adapter_backlog).map((p)=>({
  rank:p.priority_rank,source_id:p.source_id,display_name:p.display_name,verified_assignment_count:p.verified_assignment_count,
  target_claims:p.target_claims,rights_eligibility_state:p.rights_eligibility_state,
  rights_eligibility_reason_codes:p.rights_eligibility_reason_codes,rights_evidence_refs:p.rights_evidence_refs,
  next_action:'PURPOSE_SPECIFIC_RIGHTS_AND_COMMERCIAL_ACCESS_PREFLIGHT',adapter_backlog_eligible:false
}));
const backlog={
  id:'kidults-asi-source-adapter-development-backlog-v1',version:'1.0.0',state:eligibleItems.length?'RIGHTS_CLEAR_PROFILES_ONLY':'RIGHTS_GATED_NO_ELIGIBLE_PROFILE',
  backlog_count:eligibleItems.length,items:eligibleItems.map(p=>({
    ...
    rights_state:p.rights_eligibility_state,live_schema_state:'UNVERIFIED',adapter_activated:false,evidence_admitted:0,public_release:'HOLD',production:'HOLD'
  })).sort((a,b)=>a.rank-b.rank||a.source_id.localeCompare(b.source_id)),
  rights_preflight_queue_count:rightsQueue.length,
  rights_preflight_queue:rightsQueue,
  implementation_priority_rule:'RIGHTS_CLEAR_FOR_PURPOSE_REQUIRED_BEFORE_ADAPTER_BACKLOG_OR_ACQUISITION_PRIORITY',
  public_release:'HOLD',production:'HOLD'
};
...
const manifest={
  ...
  input_bindings:{runtime_contract:{id:runtime.id,digest:hash(stableJson(runtime))},frontier:{records:frontier.length,digest:hash(frontierText)},reference_contract:{id:reference.id,digest:hash(stableJson(reference))},template_contract:{id:contract.id,digest:hash(stableJson(contract))},rights_preflight:{id:rightsPreflight.id,digest:hash(stableJson(rightsPreflight))}},
  results:{registered_profiles:profiles.length,reference_adapters_implemented:1,templates_generated:16,templates_pending_implementation:15,rights_clear_profiles:registry.rights_clear_profile_count,rights_hold_profiles:registry.rights_hold_profile_count,adapter_backlog_eligible_profiles:registry.adapter_backlog_eligible_profile_count,rights_preflight_queue:backlog.rights_preflight_queue_count,source_specific_adapters_activated:0,live_source_snapshots_verified:0,evidence_admitted:0,market_events_created:0},
