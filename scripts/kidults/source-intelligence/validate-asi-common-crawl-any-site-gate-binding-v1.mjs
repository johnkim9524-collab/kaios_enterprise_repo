...
const validObservedIndex=value=>value===null||(/^CC-MAIN-\d{4}-\d{2,}$/.test(String(value||''))&&typeof value==='string');
const indexResolutionStates=new Set(['INDEX_API_RESOLVED','INDEX_ID_RESOLVED_API_UNAVAILABLE','INDEX_DISCOVERY_UNAVAILABLE']);
...
function validate(x){
  ...
  if(typeof x.common_crawl_index_api_available!=='boolean'||!indexResolutionStates.has(x.common_crawl_index_resolution_state))fail('INDEX_RESOLUTION_STATE');
  const expectedResolution=x.common_crawl_index_api_available?'INDEX_API_RESOLVED':x.common_crawl_observed_index_id?'INDEX_ID_RESOLVED_API_UNAVAILABLE':'INDEX_DISCOVERY_UNAVAILABLE';
  if(x.common_crawl_index_resolution_state!==expectedResolution)fail('INDEX_RESOLUTION_BINDING');
  ...
  if(lane.common_crawl_index_resolution_state!==x.common_crawl_index_resolution_state||lane.index_api_available!==x.common_crawl_index_api_available)fail('INDEX_RESOLUTION_LANE_BINDING');
...
function selfTest(){
  ...
  const zero={id:'kidults-asi-global-low-risk-discovery-v1',primary_target:'GLOBAL_ANY_SITE_SOURCE_UNIVERSE',common_crawl_host_expansion_applied:true,common_crawl_expansion_status:zeroStatus,common_crawl_zero_result_noop:true,common_crawl_premerge_candidate_count:2,common_crawl_seed_host_count:2,common_crawl_observed_candidate_count:0,common_crawl_new_candidate_count:0,candidate_count:2,common_crawl_index_id:null,common_crawl_observed_index_id:'CC-MAIN-2026-34',common_crawl_index_resolution_state:'INDEX_API_RESOLVED',common_crawl_index_api_available:true,common_crawl_rights_effect:'NONE',common_crawl_admission_effect:'NONE',common_crawl_acquisition_effect:'NONE',production:'HOLD',public_release:'HOLD',acquisition_authorized:false,content_acquired:false,target_site_body_crawled:false,listing_is_not_sold:true,terminal_transaction_assertion_required:true,lane_health:[{lane_id:'COMMON_CRAWL_URL_INDEX_HOST_EXPANSION',status:'SUCCESS_ZERO_RESULTS',observed_candidates:0,new_candidates:0,common_crawl_index_id:null,common_crawl_observed_index_id:'CC-MAIN-2026-34',common_crawl_index_resolution_state:'INDEX_API_RESOLVED',index_api_available:true,expansion_status:zeroStatus}],candidates:baseCandidates};
  validate(structuredClone(zero));
  const zeroNoIndex=structuredClone(zero);zeroNoIndex.common_crawl_observed_index_id=null;zeroNoIndex.common_crawl_index_resolution_state='INDEX_DISCOVERY_UNAVAILABLE';zeroNoIndex.common_crawl_index_api_available=false;zeroNoIndex.lane_health[0].common_crawl_observed_index_id=null;zeroNoIndex.lane_health[0].common_crawl_index_resolution_state='INDEX_DISCOVERY_UNAVAILABLE';zeroNoIndex.lane_health[0].index_api_available=false;validate(zeroNoIndex);
  for(const [name,mutate] of [
    ...
    ['ZERO_OBSERVED_INDEX_INVALID_FORMAT',x=>{x.common_crawl_observed_index_id='FABRICATED';x.lane_health[0].common_crawl_observed_index_id='FABRICATED';}],
    ['ZERO_INDEX_RESOLUTION_DRIFT',x=>{x.common_crawl_index_resolution_state='INDEX_DISCOVERY_UNAVAILABLE';}],
  ...
  material.common_crawl_expansion_status=materialStatus;material.common_crawl_zero_result_noop=false;material.common_crawl_index_id='CC-MAIN-2026-34';material.common_crawl_observed_index_id='CC-MAIN-2026-34';material.common_crawl_observed_candidate_count=1;material.common_crawl_new_candidate_count=1;material.candidate_count=3;material.lane_health[0]={lane_id:'COMMON_CRAWL_URL_INDEX_HOST_EXPANSION',status:'SUCCESS_WITH_RESULTS',observed_candidates:1,new_candidates:1,common_crawl_index_id:'CC-MAIN-2026-34',common_crawl_observed_index_id:'CC-MAIN-2026-34',common_crawl_index_resolution_state:'INDEX_API_RESOLVED',index_api_available:true,expansion_status:materialStatus};
  ...
  console.log(JSON.stringify({test:'COMMON_CRAWL_ANY_SITE_GATE_BINDING_SELF_TEST',state:'VERIFIED_PASS',zero_result_noop:true,zero_result_observed_index_provenance:true,material_result_binding:true,negative_cases_rejected:10,production:'HOLD'}));
}
