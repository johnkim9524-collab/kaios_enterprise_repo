...
function validate(x){
  ...
  if(zeroResults){
    if(x.common_crawl_index_id!==null)fail('ZERO_RESULTS_INDEX_ID');
    ...
    if(!x.common_crawl_index_id)fail('MATERIAL_INDEX_ID');
...
function selfTest(){
  ...
  material.common_crawl_expansion_status=materialStatus;material.common_crawl_zero_result_noop=false;material.common_crawl_index_id='CC-MAIN-TEST';material.common_crawl_observed_candidate_count=1;material.common_crawl_new_candidate_count=1;material.candidate_count=3;material.lane_health[0]={lane_id:'COMMON_CRAWL_URL_INDEX_HOST_EXPANSION',status:'SUCCESS_WITH_RESULTS',observed_candidates:1,new_candidates:1,common_crawl_index_id:'CC-MAIN-TEST',expansion_status:materialStatus};
  ...
  console.log(JSON.stringify({test:'COMMON_CRAWL_ANY_SITE_GATE_BINDING_SELF_TEST',state:'VERIFIED_PASS',zero_result_noop:true,material_result_binding:true,negative_cases_rejected:6,production:'HOLD'}));
}
...
if(process.argv.includes('--self-test'))selfTest();
  ...
  console.log(JSON.stringify({status:result.zeroResults?'PASS_ZERO_RESULTS_NOOP':'PASS_WITH_RESULTS',expansion_status:x.common_crawl_expansion_status,zero_result_noop:x.common_crawl_zero_result_noop,index_id:x.common_crawl_index_id,seed_hosts:x.common_crawl_seed_host_count,observed_candidates:x.common_crawl_observed_candidate_count,new_candidates:x.common_crawl_new_candidate_count,merged_candidates:x.candidate_count,common_crawl_bound_candidates:result.ccAny,production:'HOLD'},null,2));
}
