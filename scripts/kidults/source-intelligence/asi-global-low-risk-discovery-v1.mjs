...
try{
 ...
 for(const s of scopeNames.slice(0,16)){
  ...
  const d=await fetchJson(u,{headers:{Accept:'application/vnd.api+json','User-Agent':'KIDULTS-ASI-Any-Site-Discovery-v3'}});
...
try{
 ...
 for(const s of scopeNames){
  ...
  const sr=await fetchJson(u,{headers:{'User-Agent':'KIDULTS-ASI-Any-Site-Discovery-v3'}});const ids=(sr.search||[]).map(x=>x.id).filter(Boolean);if(!ids.length)continue;
  ...
  const d=await fetchJson(e,{headers:{'User-Agent':'KIDULTS-ASI-Any-Site-Discovery-v3'}});
