...
assert(registry.automatic_activation?.main_push===true&&registry.automatic_activation?.schedule==='52 * * * *','AUTO_ACTIVATION');
...
for(const marker of ['workflow_dispatch:','schedule:',"cron: '52 * * * *'",'push:','pull_request:','workflow_run:',"'KIDULTS ASI P0B Bounded Discovery Candidates v1'","'KIDULTS ASI P1 Source Preflight v1'",'Restore latest current P0B and P1 artifacts','Build owned source-intelligence graph twice','Reject source-candidate-as-evidence mutation','Reject market-event node mutation','Reject evidence-admission mutation','Reject orphan-edge mutation','Reject graph-digest mutation','Reject manual-only P2 activation mutation','Emit KPMO P2 owned-value receipt'])assert(workflow.includes(marker),`WORKFLOW_MARKER:${marker}`);
