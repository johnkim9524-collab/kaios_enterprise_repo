...
for(const marker of ['workflow_dispatch:','pull_request:','workflow_run:',"'KIDULTS ASI P1 Source Preflight v1'",'Restore transactionally paired P0B and P1 inputs from one P1 run','/actions/runs/${P1_RUN_ID}/artifacts','git merge-base --is-ancestor','SAME_SUCCESSFUL_P1_WORKFLOW_RUN','Build owned source-intelligence graph twice','Reject source-candidate-as-evidence mutation','Reject market-event node mutation','Reject evidence-admission mutation','Reject orphan-edge mutation','Reject graph-digest mutation','Reject manual-only P2 activation mutation','Emit KPMO P2 owned-value receipt'])assert(workflow.includes(marker),`WORKFLOW_MARKER:${marker}`);
...
console.log(JSON.stringify({id:'kidults-asi-owned-source-intelligence-graph-registry-validation-v2',state:'VERIFIED_PASS',principles,node_types:13,edge_types:14,outputs:5,automatic_main_push:false,automatic_schedule:null,automatic_upstream_workflows:1,transactional_input_binding:'P0B_AND_P1_ARTIFACTS_FROM_ONE_SUCCESSFUL_P1_RUN',direct_repository_mutation_from_workflow:false,public_release:'HOLD',production:'HOLD'},null,2));
