...
for(const cell of x.cells){if(cell.collection_quota!==null||cell.analytical_weight!==null||cell.eligibility!=='NOT_VERIFIED') fail(`premature regional activation:${cell.macroregion_id}`);for(const [name,v] of Object.entries(cell.factors||{})){if(v?.state==='UNKNOWN') unknown++; else verified++;if(v?.state==='UNKNOWN'&&(v?.evidence_refs?.length||v?.provenance_refs?.length)) fail(`unknown factor has fabricated evidence:${cell.macroregion_id}:${name}`);}}
