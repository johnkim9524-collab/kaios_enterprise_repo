...
if (manifest.product_identity_context_rows !== 1280) fail('all demand rows must contain Product identity context');
...
let rows=0, decisionBound=0, valueBound=0, globalBound=0, identityBound=0;
...
for (const s of manifest.shards) {
  ...
  for (const r of p.records) {
    ...
    for (const key of ['representative_product_id','maker_or_brand','product_name','display_name','identity_level','category_id','collection_scope_id','market_cell_id','assertion_id','evidence_gap_class','required_source_roles','independent_family_floor','rights_requirement','owner_lineage_diversity_requirement','queue_state']) {
    ...
    decisionBound++; valueBound++; globalBound++; identityBound++;
  }
}
if (rows !== manifest.demand_records) fail(`row total mismatch ${rows}`);
if (scopes.size !== 32) fail(`scope coverage mismatch ${scopes.size}`);
if (decisionBound !== rows || valueBound !== rows || globalBound !== rows || identityBound !== rows) fail('binding coverage must be 100%');
console.log(JSON.stringify({status:'PASS',rows,scopes:scopes.size,identityBound,decisionBound,valueBound,globalBound,north_star:manifest.north_star,discovery_active:manifest.discovery_active,acquisition_authorized:manifest.acquisition_authorized,production:manifest.production},null,2));
