...
const [outputDir,candidateRegistryPath,bindingLedgerPath,gate1Path,admissionPath,actionQueuePath,frontierPath,crosswalkPath,adapterContractPath,contractPath] = process.argv.slice(2);
if (![outputDir,candidateRegistryPath,bindingLedgerPath,gate1Path,admissionPath,actionQueuePath,frontierPath,crosswalkPath,adapterContractPath,contractPath].every(Boolean)) throw new Error('AUTONOMOUS_RESOLUTION_VALIDATION_ARGUMENTS_REQUIRED');
...
const inputs={
  ...
  actions:json(actionQueuePath),frontier:parsePsv(text(frontierPath)),crosswalk:json(crosswalkPath),adapter:json(adapterContractPath),contract:json(contractPath)
};
...
for(const m of replacement.missions){assert(m.slots?.length===3&&m.rights_or_admission_created===false,'REPLACEMENT_MISSION_BOUNDARY');for(const s of m.slots.filter((x)=>x.source_id)){assert(profileIds.has(s.source_id)&&frontierIds.has(s.source_id),'REPLACEMENT_PROFILE_SOURCE');assert(s.adapter_state==='ADAPTER_NOT_IMPLEMENTED'&&s.rights_state==='UNKNOWN'&&s.sold_or_liquidity_semantics_state==='UNVERIFIED'&&s.factual_origin_independence_state==='UNVERIFIED'&&s.evidence_admitted===false,'REPLACEMENT_PROFILE_STATE');}}
for(const b of replacement.adapter_development_backlog){assert(profileIds.has(b.source_id)&&frontierIds.has(b.source_id)&&b.required_next_steps?.length===7,'BACKLOG_PROFILE');assert(b.adapter_state==='ADAPTER_NOT_IMPLEMENTED'&&b.rights_state==='UNKNOWN'&&b.semantic_state==='UNVERIFIED'&&b.factual_origin_state==='UNVERIFIED'&&b.evidence_admitted===false,'BACKLOG_STATE');}
