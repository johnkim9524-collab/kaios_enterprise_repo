...
assert(replacement.registered_profile_is_rights_verified===false&&replacement.registered_profile_is_adapter_implemented===true&&replacement.evidence_admitted===0,'REPLACEMENT_IMPLEMENTATION_AND_PROMOTION_BOUNDARY');
...
for(const m of replacement.missions){assert(m.slots?.length===3&&m.rights_or_admission_created===false,'REPLACEMENT_MISSION_BOUNDARY');assert(m.rights_clear_registered_profile_count===m.eligible_registered_profile_count,'MISSION_RIGHTS_COUNT');for(const s of m.slots.filter((x)=>x.source_id)){assert(profileIds.has(s.source_id)&&frontierIds.has(s.source_id),'REPLACEMENT_PROFILE_SOURCE');assert(s.rights_eligibility_state===RIGHTS_CLEAR&&s.adapter_state==='IMPLEMENTED_NOT_RIGHTS_VERIFIED'&&s.sold_or_liquidity_semantics_state==='UNVERIFIED'&&s.factual_origin_independence_state==='UNVERIFIED'&&s.evidence_admitted===false,'REPLACEMENT_PROFILE_STATE');}}
for(const b of replacement.adapter_development_backlog){assert(profileIds.has(b.source_id)&&frontierIds.has(b.source_id)&&b.required_next_steps?.length===6,'BACKLOG_PROFILE');assert(b.adapter_state==='IMPLEMENTED_NOT_RIGHTS_VERIFIED'&&b.rights_eligibility_state===RIGHTS_CLEAR&&b.semantic_state==='UNVERIFIED'&&b.factual_origin_state==='UNVERIFIED'&&b.evidence_admitted===false,'BACKLOG_STATE');}
