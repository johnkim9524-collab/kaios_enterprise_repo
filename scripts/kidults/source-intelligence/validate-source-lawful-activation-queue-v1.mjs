...
assert(queue.activation_queue.length + array(queue.deferred_actionable).length + queue.dropped_or_wrong_purpose.length === queue.funnel.discovered_or_preflighted, 'SOURCE_PACKET_ACCOUNTABILITY_GAP');

const validateNonAuthority = packet => {
...
for (const packet of array(queue.deferred_actionable)) {
  validateNonAuthority(packet);
  assert(packet.lane !== 'DROP_OR_WRONG_PURPOSE', `DEFERRED_WRONG_PURPOSE:${packet.source_id}`);
  assert(!queue.activation_queue.some(active => active.source_id === packet.source_id), `DEFERRED_SOURCE_DUPLICATED_IN_QUEUE:${packet.source_id}`);
}
...
console.log(`DEFERRED_ACTIONABLE_COUNT=${array(queue.deferred_actionable).length}`);
