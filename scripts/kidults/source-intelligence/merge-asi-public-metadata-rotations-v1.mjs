...
const rotationIds=new Set();const scopeIds=new Set();const byUrl=new Map();const laneAgg=new Map();
for(const x of rotations){
 ...
 if(!Array.isArray(x.cycle_scope_ids)||x.cycle_scope_ids.length!==8)fail('ROTATION_SCOPE_COUNT');for(const s of x.cycle_scope_ids){if(scopeIds.has(s))fail(`SCOPE_OVERLAP:${s}`);scopeIds.add(s)}
 for(const lane of x.lane_health||[]){
  const a=laneAgg.get(lane.lane_id)||{lane_id:lane.lane_id,observed_candidates:0,successful_rotation_count:0,failed_rotation_count:0,zero_result_rotation_count:0,errors:[]};
 ...
 for(const raw of x.candidates||[]){
  ...
  const scopes=[...new Set((raw.scope_hints||[raw.scope_hint]).filter(Boolean))];
...
if(rotationIds.size!==4||scopeIds.size!==32)fail(`FULL_SCOPE_SWEEP_REQUIRED:${rotationIds.size}:${scopeIds.size}`);
const laneHealth=[...laneAgg.values()].map(a=>{const anySuccess=a.successful_rotation_count+a.zero_result_rotation_count>0;const status=a.successful_rotation_count>0?'SUCCESS_WITH_RESULTS':a.zero_result_rotation_count>0?'SUCCESS_ZERO_RESULTS':'FAILED';return{...a,status,error:a.errors.length?a.errors.join('|').slice(0,500):null,degraded:anySuccess&&a.failed_rotation_count>0};}).sort((a,b)=>a.lane_id.localeCompare(b.lane_id));
...
const output={id:'kidults-asi-public-metadata-source-fabric-v1',version:'1.0.0',status:'SHADOW_PUBLIC_METADATA_SOURCE_FABRIC_INCREMENT_READY',universe_target:'GLOBAL_ANY_SITE_SOURCE_UNIVERSE',universe_restricted:false,rotation_count:4,covered_scope_count:scopeIds.size,covered_scope_ids:[...scopeIds].sort(),lane_health:laneHealth,healthy_lane_count:laneHealth.filter(l=>['SUCCESS_WITH_RESULTS','SUCCESS_ZERO_RESULTS'].includes(l.status)).length,degraded_lane_count:laneHealth.filter(l=>l.degraded===true).length,candidate_count:candidates.length,live_external_candidate_count:candidates.length,unique_host_count:hosts.size,provider_counts:providerCounts,candidates,rules:{full_32_scope_sweep:true,multi_provider_fail_soft:true,discovery_metadata_only:true,attention_is_not_demand:true,listing_is_not_sold:true,terminal_transaction_assertion_required:true,reserve_is_not_safe_pool:true,rights_never_promoted:true,admission_never_promoted:true,target_site_body_traversal_forbidden:true,content_acquisition_forbidden:true},target_site_body_crawled:false,content_acquired:false,acquisition_authorized:false,public_release:'HOLD',production:'HOLD'};
fs.mkdirSync(path.dirname(out),{recursive:true});fs.writeFileSync(out,JSON.stringify(output,null,2)+'\n');
console.log(JSON.stringify({status:output.status,rotations:4,scopes:output.covered_scope_count,candidates:output.candidate_count,hosts:output.unique_host_count,healthy_lanes:output.healthy_lane_count,degraded_lanes:output.degraded_lane_count,production:'HOLD'}));
