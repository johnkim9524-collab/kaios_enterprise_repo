...
if(!Array.isArray(x.providers)||x.providers.length!==Number(x.summary?.managed_provider_count))fail('PROVIDER_COUNT');
const actions=new Set(['FULL_DISCOVERY_BUDGET','SKIP_PROVIDER_NEXT_CYCLE','SINGLE_BOUNDED_PROBE','ON_DEMAND_ONLY']);const states=new Set(['CLOSED_HEALTHY','OPEN_COOLDOWN','HALF_OPEN_PROBE','CLOSED_ON_DEMAND']);
for(const p of x.providers){
 ...
 if(p.discovery_only!==true||p.rights_effect!=='NONE'||p.admission_effect!=='NONE'||p.production!=='HOLD')fail('PROVIDER_PERMISSION_BOUNDARY');
 if(p.current_status==='FAILED'&&p.next_cycle_action==='FULL_DISCOVERY_BUDGET')fail('FAILED_PROVIDER_FULL_BUDGET');
 ...
 if(['SUCCESS_WITH_RESULTS','SUCCESS_ZERO_RESULTS'].includes(p.current_status)&&(p.state!=='CLOSED_HEALTHY'||p.next_cycle_action!=='FULL_DISCOVERY_BUDGET'||Number(p.cooldown_cycles_remaining)!==0))fail('SUCCESS_DID_NOT_RESET');
 if(p.state==='HALF_OPEN_PROBE'&&p.next_cycle_action!=='SINGLE_BOUNDED_PROBE')fail('HALF_OPEN_NOT_BOUNDED');
}
if(x.summary?.all_provider_circuits_open===true&&g.mode!=='MINIMAL_INDEPENDENT_PROBE_RECOVERY')fail('ALL_OPEN_RECOVERY_MODE');
console.log(JSON.stringify({status:'PASS',cycle:x.cycle_number,providers:x.providers.length,open:x.summary.open_circuit_count,half_open:x.summary.half_open_probe_count,mode:g.mode,production:'HOLD'}));
