...
async function fetchJson(url,opts={},attempt=0){
 ...
 try{const r=await fetch(url,{...opts,signal:controller.signal});if((r.status===429||r.status>=500)&&attempt<2){await sleep(900*(2**attempt));return fetchJson(url,opts,attempt+1)}if(!r.ok)throw new Error(`HTTP_${r.status}`);const type=r.headers.get('content-type')||'';if(!type.includes('json'))throw new Error(`NON_JSON_RESPONSE:${type.slice(0,40)}`);return await r.json();}finally{clearTimeout(timer)}
}
