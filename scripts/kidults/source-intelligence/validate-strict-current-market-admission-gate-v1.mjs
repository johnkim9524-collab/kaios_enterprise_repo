...
for (const key of ['CLAIM_SPECIFIC_FRESHNESS_THRESHOLD_CALIBRATED','CONDITION_OR_GRADE_SEGMENTATION','MINIMUM_SAMPLE_RULE_CALIBRATED']) {
  if (!(current.required || []).includes(key)) throw new Error(`CURRENT_PRICE_REQUIREMENT_MISSING_${key}`);
}
for (const key of ['EXPOSURE_DENOMINATOR','SALE_OR_CENSOR_END','FAILED_SALE_OR_CENSOR_HANDLING']) {
  if (!(liquidity.required || []).includes(key)) throw new Error(`LIQUIDITY_REQUIREMENT_MISSING_${key}`);
}
...
for (const claim of ['CURRENT_PRICE','REPRESENTATIVE_PRICE','LIQUIDITY','TIME_TO_SALE','GLOBAL_DEMAND','GLOBAL_REPRESENTATIVENESS','CONDITION_ADJUSTED_VALUE','PUBLIC_OR_COMMERCIAL_PROJECTION']) {
  if (!blocked.has(claim)) throw new Error(`COLLECTAIO_BLOCKED_CLAIM_MISSING_${claim}`);
}
...
console.log(JSON.stringify({
  ...
  validator: 'KIDULTS_STRICT_CURRENT_MARKET_ADMISSION_GATE_V1',
  current_empirical_source: collectaio.id,
  classification: 'DATED_ONLY_NOT_STRICT_CURRENT',
  strict_current_price_eligible: false,
  liquidity_eligible: false,
  public_or_commercial_projection_eligible: false,
  production: 'HOLD'
}, null, 2));
