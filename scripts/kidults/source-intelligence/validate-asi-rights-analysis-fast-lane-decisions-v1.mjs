...
function validateSnapshots(document,snapshots){
  ...
  assert(snapshots.status==='FAIL_CLOSED_REFERENCE_ONLY_AND_AUTHORIZED_CAPTURE_PENDING','SNAPSHOT_MANIFEST_STATUS');
  ...
  const counts={SOURCE_CONTENT_SNAPSHOT_BOUND:0,AUTHORIZED_CAPTURE_PENDING:0,REFERENCE_ONLY_NOT_CAPTURED_DUE_RESTRICTION:0};
  for(const snapshot of snapshots.records){
    ...
    if(snapshot.capture_state==='SOURCE_CONTENT_SNAPSHOT_BOUND'){
      ...
      if(snapshot.capture_state==='AUTHORIZED_CAPTURE_PENDING')assert(decision.decision==='HOLD'&&snapshot.retention_and_deletion_class==='PENDING_RIGHTS_APPROVED_CAPTURE_CLASS',`CAPTURE_PENDING_REQUIRES_HOLD:${snapshot.source_id}`);
    }
  }
  assert(snapshots.summary.sources===snapshots.records.length&&snapshots.summary.source_content_snapshot_bound===counts.SOURCE_CONTENT_SNAPSHOT_BOUND&&snapshots.summary.authorized_capture_pending===counts.AUTHORIZED_CAPTURE_PENDING&&snapshots.summary.reference_only_not_captured_due_restriction===counts.REFERENCE_ONLY_NOT_CAPTURED_DUE_RESTRICTION,'SNAPSHOT_SUMMARY_COUNTS');
...
dynamicSnapshot.capture_state='AUTHORIZED_CAPTURE_PENDING';
...
dynamicSnapshots.summary.authorized_capture_pending++;
...
expectFailure('SNAPSHOT_RETRIEVED_AT:',(_candidate,snapshots)=>{const record=snapshots.records.find(item=>item.capture_state==='AUTHORIZED_CAPTURE_PENDING');record.capture_state='SOURCE_CONTENT_SNAPSHOT_BOUND';record.capture_authorized=true;record.decision_promotion_eligible=true;snapshots.summary.authorized_capture_pending--;snapshots.summary.source_content_snapshot_bound++;snapshots.summary.promotion_eligible++});
