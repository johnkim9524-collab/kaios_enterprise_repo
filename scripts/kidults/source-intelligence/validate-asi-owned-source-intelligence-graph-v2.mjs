...
assert(counts.GATE1_DECISION>0,'GATE1_NODE_COUNT');
assert(counts.ADMISSION_CANDIDATE===counts.GATE1_DECISION,'ADMISSION_NODE_COUNT');
assert(counts.PREFLIGHT_ACTION>0&&counts.ACTION_TYPE===p1Contract.preflight_actions.length,'ACTION_NODE_COUNT');
const graphActionTypes=graph.nodes.filter(node=>node.node_type==='ACTION_TYPE').map(node=>node.properties?.action_type).sort();
assert(JSON.stringify(graphActionTypes)===JSON.stringify([...p1Contract.preflight_actions].sort()),'ACTION_TYPE_CONTRACT');
assert(edgeCounts.MISSION_HAS_SOURCE_CANDIDATE===counts.GATE1_DECISION,'MISSION_CANDIDATE_EDGE_COUNT');
assert(edgeCounts.CANDIDATE_HAS_GATE1_DECISION===counts.GATE1_DECISION&&edgeCounts.MISSION_HAS_GATE1_DECISION===counts.GATE1_DECISION,'GATE1_EDGE_COUNT');
assert(edgeCounts.CANDIDATE_HAS_ADMISSION_CANDIDATE===counts.ADMISSION_CANDIDATE&&edgeCounts.MISSION_HAS_ADMISSION_CANDIDATE===counts.ADMISSION_CANDIDATE,'ADMISSION_EDGE_COUNT');
assert(edgeCounts.CANDIDATE_REQUIRES_PREFLIGHT_ACTION===counts.PREFLIGHT_ACTION&&edgeCounts.ACTION_IS_TYPE===counts.PREFLIGHT_ACTION,'ACTION_EDGE_COUNT');
...
assert(manifest.results.gate1_decisions===counts.GATE1_DECISION&&manifest.results.admission_candidates===counts.ADMISSION_CANDIDATE&&manifest.results.preflight_actions===counts.PREFLIGHT_ACTION,'MANIFEST_P1_COUNTS');
