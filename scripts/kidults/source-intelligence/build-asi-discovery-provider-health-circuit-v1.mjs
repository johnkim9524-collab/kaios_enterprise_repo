...
for(const lane of d.lane_health||[]){
 ...
 if(success.has(lane.status)){state='CLOSED_HEALTHY';action='FULL_DISCOVERY_BUDGET';reason='CURRENT_CYCLE_HEALTHY';}
 else if(neutral.has(lane.status)){state='CLOSED_ON_DEMAND';action='ON_DEMAND_ONLY';reason='NOT_SCHEDULED_OR_NOT_TARGETED';}
 ...
 }else{state='CLOSED_ON_DEMAND';action='ON_DEMAND_ONLY';reason='UNRECOGNIZED_NON_FAILURE_STATE';}
