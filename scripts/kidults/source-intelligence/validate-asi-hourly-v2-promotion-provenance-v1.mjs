...
function assertions(text) {
  ...
  const empiricalRestoreBlock = text.match(/- name: Bind triggering run and restore two exact-generation v2 cycles([\s\S]*?)- name: Emit waiting evidence/)?.[1] ?? '';
  ...
  require(empiricalRestoreBlock.length > 0, 'empirical restore block missing');
  require(empiricalRestoreBlock.includes('test "$MAIN_SHA" = "$EXPECTED_SHA"'), 'current protected-main equality proof missing');
  ...
  require(empiricalRestoreBlock.includes('WAITING_FOR_SECOND_DISTINCT_EXACT_SHA_V2_CYCLE'), 'exact-generation waiting state missing');
  require(empiricalRestoreBlock.includes('test "$SECOND_RUN_ID" != "$UPSTREAM_RUN_ID"'), 'distinct-run proof missing');
  require(empiricalRestoreBlock.includes('"triggering_run_included": true'), 'triggering-run inclusion receipt missing');
  require(empiricalRestoreBlock.includes('"same_generation": true'), 'same-generation receipt missing');
  ...
  const safeValidation = empiricalRestoreBlock.indexOf('python3 scripts/kidults/kpmo/validate-safe-zip-archive-v1.py');
  const extraction = empiricalRestoreBlock.indexOf('unzip -q -o "/tmp/v2-${i}.zip"');
  require(safeValidation >= 0 && extraction >= 0 && safeValidation < extraction, 'safe ZIP validation must precede extraction');
  require(empiricalRestoreBlock.includes('--required-basename asi-global-any-site-hourly-cycle-receipt-v2.json'), 'exact receipt basename archive guard missing');
  require(empiricalRestoreBlock.includes('"safe_zip_validated_before_extraction": true'), 'safe ZIP provenance state missing');
  require(empiricalRestoreBlock.includes('safe_zip_receipt_digest'), 'safe ZIP receipt digest binding missing');
  return errors;
}
...
const mutations = [
  ...
  ['remove-safe-basename', source.replace('              --required-basename asi-global-any-site-hourly-cycle-receipt-v2.json', '              --max-entries 2048 # removed required basename')],
