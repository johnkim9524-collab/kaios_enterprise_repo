...
function read(relative, base = registryRoot) {
  try { return JSON.parse(fs.readFileSync(path.join(base, relative), "utf8")); }
...
function semverAtLeast(actual, minimum) {
  const a = String(actual ?? "0.0.0").split(".").map(Number);
  const m = String(minimum).split(".").map(Number);
  for (let i=0;i<3;i+=1) { if ((a[i]??0) > (m[i]??0)) return true; if ((a[i]??0) < (m[i]??0)) return false; }
  return true;
}
...
for (const required of ["autonomous","engine","memory","raw-quarantine","universe","core-domain","entity","market-event","market-graph","cluster","dynamic-vertical","index","provider-mapping","projection"]) {
...
assert(memoryRun?.provenance_coverage === 1 && memoryRun?.rights_coverage === 1 && memoryRun?.bitemporal_coverage === 1, "Memory coverage must remain 100%.");
assert(memoryRun?.fixture_entries_in_global_universe === false && memoryRun?.indexes_computed === 0 && memoryRun?.production_eligible === false, "Memory fixture boundary violation.");
...
assert(coreDomains?.not_permanent_market_verticals === true && coreDomains?.fixed_index_allocation === false, "Core Domain boundary mismatch.");
assert(legacyVertical?.record_count === 8, "Legacy eight Vertical records must remain intact for compatibility.");
for (const domain of coreDomains?.domains ?? []) assert(legacyVertical?.records?.some(item => item.id === domain.legacy_vertical_id), `${domain.id}: legacy Vertical alias does not resolve.`);
...
assert(marketEvent?.listing_is_sale === false && marketEvent?.direct_portal_projection === false, "Market Event boundary mismatch.");
assert(marketGraph?.provider_category_is_graph_cluster === false, "Provider categories cannot define market clusters.");
assert(cluster?.autonomous_recommendation === true && cluster?.autonomous_public_promotion === false, "Cluster discovery boundary mismatch.");
...
assert(dynamicVertical?.states?.includes("SPLIT") && dynamicVertical?.states?.includes("MERGED") && dynamicVertical?.core_domain_is_vertical === false, "Dynamic Vertical lifecycle mismatch.");
...
assert(providerMapping?.direct_provider_to_portal === false && providerMapping?.direct_provider_to_index === false, "Direct Provider path must remain false.");
...
assert(semverAtLeast(projectionIndex?.registry_version, "2.4.0"), "Projection Registry version must be >=2.4.0 after semantic-freshness alignment.");
assert(projection?.semantic_freshness?.status === "CURRENT_CANONICAL_BASELINE", "Projection semantic freshness must be current.");
assert(projection?.market_funnel_alignment?.contract === "coordination/kidults/architecture/platform-market-funnel-alignment-v1.json", "Projection must bind to canonical market funnel.");
assert(projection?.market_funnel_alignment?.repository_canonical_architecture_and_integration_boundary_alignment_percent === 100, "Projection must expose 100% repository alignment.");
assert(projection?.market_funnel_alignment?.full_52_engine_runtime_implementation_verified === false && projection?.market_funnel_alignment?.deployed_operational_alignment_percent === 0, "Projection must not overclaim runtime implementation/deployment.");
assert(projection?.autonomous?.first_value === "AUTONOMOUS", "Projection must expose Autonomous first.");
assert(projection?.memory?.current_run_id === memoryRun?.id && projection?.memory?.storage_model === "APPEND_ONLY_BITEMPORAL" && projection?.memory?.deterministic_replay === "PASS", "Projection Memory mismatch.");
assert(projection?.memory?.direct_memory_to_portal === false && projection?.memory?.fixture_entries_in_global_universe === false && projection?.memory?.public_projection === false, "Projection Memory boundary violation.");
assert(projection?.core_domains?.count === 8 && projection?.dynamic_verticals?.approved_count === 0, "Projection domain/vertical mismatch.");
assert(projection?.indexes?.kidult_500?.status === "NOT_COMPUTED" && projection?.indexes?.kidult_100?.status === "NOT_COMPUTED", "Projection must not fabricate indexes.");
assert(projection?.provider?.direct_portal_path === false && projection?.publication?.production === "HOLD", "Projection provider/production boundary mismatch.");
...
assert(portal?.source_projection_id === projection?.id, "Portal must consume current Projection Registry record.");
assert(portal?.architecture?.boundary === "PROJECTION_CONSUMER_ONLY", "Portal boundary must be Projection consumer only.");
assert(portal?.freshness?.status === "CURRENT_CANONICAL_BASELINE", "Portal must consume semantically current Projection.");
...
assert(portal?.memory?.current_run_id === memoryRun?.id && portal?.memory?.direct_memory_to_portal === false && portal?.memory?.public_projection === false, "Portal Memory boundary mismatch.");
assert(portal?.provider?.direct_portal_path === false, "Portal must expose no direct Provider path.");
assert(portal?.indexes?.kidult_500?.status === "NOT_COMPUTED" && portal?.publication?.production === "HOLD", "Portal index/production boundary mismatch.");
assert(compatibility?.strategy === "STRANGLER_PATTERN" && compatibility?.legacy_preservation === true, "Compatibility strategy mismatch.");
assert(architecture.includes("AUTONOMOUS") && architecture.includes("AGCI-OS"), "Canonical architecture document incomplete.");
assert(memoryArchitecture.includes("append-only") && memoryArchitecture.includes("recorded_at") && memoryArchitecture.includes("valid_from"), "Memory architecture document incomplete.");
...
console.log(`Projection Registry: ${projectionIndex.registry_version}`);
console.log(`Projection freshness: ${projection.semantic_freshness.status}`);
