...
# Capture the recovery target strictly after the durable BEFORE marker commit.
target_time="$(psql_scalar "SELECT clock_timestamp() AT TIME ZONE 'UTC'")"
# Ensure the AFTER marker is committed after the target timestamp even on coarse clocks.
sleep 1
psql --no-psqlrc --set=ON_ERROR_STOP=1 --dbname="$KAIOS_POSTGRES_DSN" \
