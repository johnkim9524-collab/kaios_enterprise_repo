...
ROLLBACK="$ROOT_DIR/scripts/staging/rollback-dual-staging-postgres-greenfield.sh"
...
SQL
  ...
  psql --no-psqlrc --quiet --tuples-only --no-align --dbname="$KAIOS_POSTGRES_RESTORE_DSN" \
    --command="SELECT to_regnamespace('kaios_runtime') IS NULL" | grep -qx 't'
  rollback_status="pass"

  KAIOS_POSTGRES_DSN="$KAIOS_POSTGRES_RESTORE_DSN" bash "$GREENFIELD"
fi
