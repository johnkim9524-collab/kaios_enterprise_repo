...
The mandatory Track Z money-to-usable-data gate is `docs/strategy/TRACK_Z_MONEY_TO_USABLE_DATA_GATE_V1.md` and its machine contract is `coordination/kidults/governance/track-z-money-to-usable-data-gate-v1.json`.

Any agent analyzing, sourcing, contacting, contracting, integrating, monitoring, or reporting on a data provider must read the provider strategy, sourcing contract, and every mandatory addendum from the exact committed `HEAD` after passing bootstrap. It must also inspect current registry and communication evidence, avoid duplicate outreach, preserve legal/spend/credential/Production/external-communication gates, and report by source layer, brand/vertical, and provider. A previously sent message—including the eBay Developer Relations licensing inquiry—must not be resent without explicit authority. No provider payment, paid trial, auto-converting trial, contract, credential activation, or data acquisition may proceed unless the Track Z money-to-usable-data gate records complete evidence from `PAYMENT` through `PRODUCT`; any missing link fails closed as `NO_PAY_HOLD`.
