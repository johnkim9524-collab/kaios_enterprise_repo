...
20. **Operability-first completion.** Every improvement must prioritize restoring or preserving verified normal operation with minimal architectural complexity. Green checks, implementation, or a merged patch are intermediate evidence only: authorized work is not complete until the intended operational path runs end-to-end in its authorized environment, the root cause is removed, failure isolation and rollback are verified, recurrence prevention is enforced, and exact-head/target-main evidence is regenerated. A protected external gate blocks only that protected action; independent authorized remediation and verification must continue. Never weaken fail-closed evidence, rights, security, or release gates to manufacture a green status.
