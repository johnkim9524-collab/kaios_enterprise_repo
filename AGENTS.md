...
Every AI agent and automation must load and inherit the fix-first bootstrap before task execution. For any reversible internal defect within granted authority, the normal sequence is mandatory and may not be shortened to a report-only response:
...
Reporting before remediation is allowed only when a protected authority gate or missing required tool/permission prevents execution. In that case the agent must fail closed and report the exact blocker and unblock condition. No child agent, reviewer, coding agent, runtime agent, scheduled automation, or external model agent may weaken or self-exempt from this bootstrap.
