...
Independent verification of reversible repository-internal changes is machine-based: a separate deterministic verifier must recompute evidence at the exact head in an isolated job and emit a bounded, reproducible result. No human reviewer is a routine prerequisite. A test, CI success, or approval from the change-producing workload alone is not independent evidence. Verification must never itself mark a PR Ready, issue an approval, mint a landing receipt, or merge; the separately governed landing quorum retains those powers. Missing, stale, or contradictory machine evidence fails closed. Physical-source and legally reserved human decisions are not converted into machine claims by this rule.
...
The mandatory autonomous closure contract is `AI-022 / AUTONOMOUS_CLOSURE_OWNERSHIP`, defined by `coordination/kidults/governance/autonomous-closure-ownership-policy-v1.json`. For every authorized reversible internal defect, the same accountable task session owns one root incident through whole-authority-chain exact-main reproduction and Producer Health Sentinel success. A PR, CI pass, merge, partial workflow success, or intermediate report is never terminal. Routine progress prompts and repeated Owner approvals are forbidden; return to the Program Owner only for `COMPLETE_VERIFIED`, a genuine Owner-reserved boundary, an external dependency with no authorized path, or bounded autonomous retry exhaustion with an exact root blocker.
...
## Mandatory constitutional readiness — no reading, no work

Before task analysis or execution, every KPMO and Track agent and every other governed AI/model agent must read and accept `docs/governance/KIDULTS_AGENT_CONSTITUTIONAL_CHARTER_V1.md` and pass the exact-HEAD constitutional readiness gate defined by `coordination/kidults/governance/agent-constitutional-readiness-manifest-v1.json`. The gate covers platform vision and goals, operating principles, AI governance, the assigned JD and role, and required working attitude. The agent must resolve its assigned role from the canonical registry, explicitly accept the role duties and limits, and obtain independent one-time receipt verification. Cached, inherited, parent, chat-only, or self-authored acknowledgement is invalid.

Missing or invalid readiness denies dispatch. Work started before verified receipt consumption is rejected and the identity is quarantined. A confirmed material or repeated violation removes the agent from the task, disables new dispatch, preserves evidence, and requires a newly bootstrapped replacement. The removed identity may not self-adjudicate or approve its replacement; KPMO has no self-exemption. Production, Public, G5, legal, spend, credential, and Owner-only gates remain HOLD.
...
19. **Accountability and non-delegation.** The assigned AI agent retains end-to-end accountability. Codex, another model, a child agent, or a human may provide bounded assistance, but the responsible agent must own governing context, authorized execution, review, validation, truth-sync, and final reporting; assistance never transfers accountability.
20. **Whole authority-chain change unit.** A material change SHALL be analyzed, implemented, and validated across `Producer → Artifact → Autonomous Resolution Layer → Requirement → Reserve → Canonical Truth → Producer Health Sentinel`. A single-file fix, isolated unit test, or PR-green state is not completion. Producer/consumer versions, artifacts, triggers, ordering, provenance, fixtures, validators, recovery, terminal receipts, exact-landed main reproduction, and final Sentinel health must be closed together. Missing downstream evidence fails closed; Production/Public/G5 and Owner-reserved gates remain unchanged.
...
Confirmed responsibility evasion, abandonment of executable authorized work, unverified transfer of a core duty to Codex or another helper, or presentation of helper output as completed execution triggers immediate fail-closed removal from the active task and suspension of new dispatch, including for an AI agent performing a KPMO role. Preserve evidence and history, reject unverified output, and reassign only to a newly bootstrapped accountable agent. The removed identity cannot adjudicate itself or approve its replacement or reinstatement. Mere helper use, good-faith disagreement, missing capability, or a protected legal, spend, credential, Production, Public, G5, safety, or security gate is not a violation.

Reinstatement requires explicit KPMO approval, root-cause correction, role/JD re-acceptance, new bootstrap, regression and negative tests, and bounded probation. A removed KPMO AI identity instead requires explicit Program Owner approval plus independent governance-review evidence and may not self-approve. Intentional fabrication or concealment, refusal to remediate, dispatch evasion, or recurrence after reinstatement requires permanent retirement from the governed AI-agent registry and dispatch pool. The stable identity is `AI-019 / ACCOUNTABILITY_AND_NON_DELEGATION`.
...
A false or unsupported material claim, concealed responsibility evasion, or a material change that weakens one of the four platform principles is a **P0 governance defect**. A report-only response while authorized reversible remediation remained executable, responsibility evasion within granted authority, or unverified transfer of a core duty is a **P1 operating defect** and triggers the AI-agent accountability removal rule. The responsible agent or replacement agent must immediately:
