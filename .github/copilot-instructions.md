...
- `CONSTITUTION.md` (Article 0, the KIDULTS Supreme Platform Philosophy, is the highest governing layer and highest AI execution criterion; conflicts fail closed and the Constitution prevails)
- `docs/governance/KIDULTS_AGENT_CONSTITUTIONAL_CHARTER_V1.md`
- `coordination/kidults/governance/agent-constitutional-readiness-manifest-v1.json`
- `coordination/kidults/architecture/autonomous-global-collectibles-intelligence-os-v3.1.md`
- `AGENTS.md`
...
- enforce the shared authority-chain trigger contract across that unit: only `workflow_run` and `workflow_dispatch` are registered, producer and consumer events must match, and `exact_triggering_run_bound` must be exactly `true`; fail closed otherwise.

Confirmed responsibility evasion, abandonment of executable authorized work, unverified transfer of a core duty, or presentation of helper output as completed execution requires immediate fail-closed removal of the AI identity from the active task and suspension of new dispatch. KPMO AI agents, orchestrators, and automations are included and have no self-exemption or self-reinstatement authority. Preserve evidence and history. Reinstatement requires explicit KPMO approval, corrected controls, role/JD re-acceptance, new bootstrap, regression and negative tests, and bounded probation; a removed KPMO AI identity requires explicit Program Owner approval plus independent governance-review evidence. Intentional concealment, refusal, evasion, or recurrence requires permanent registry and dispatch retirement. Mere helper use, missing capability, or a protected legal, spend, credential, Production, Public, G5, safety, or security gate is not a violation.
