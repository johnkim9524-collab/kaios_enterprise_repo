...
class InMemoryRateLimiter:
    ...
    def check(self, key: str) -> RateLimitResult:
        ...
        with self._lock:
            ...
            events.append(now)
            return RateLimitResult(True, self.limit, max(0, self.limit - len(events)), 0)


class DurableRateLimiter:
    """SQLite-backed limiter whose decisions survive process restarts.

    Request-slot decisions are serialized with ``BEGIN IMMEDIATE``. WAL mode
    is configured during bootstrap only; normal request connections never
    renegotiate journal mode, avoiding concurrent constructor lock races.
    """

    _INITIALIZE_ATTEMPTS = 20
    _INITIALIZE_RETRY_SECONDS = 0.05

    def __init__(self, limit: int, window_seconds: int, path: Path | str) -> None:
        self.limit = limit
        self.window_seconds = window_seconds
        self.path = Path(path)
    ...
    def check(self, key: str) -> RateLimitResult:
        ...
        try:
            ...
            if len(rows) >= self.limit:
                retry_after = max(1, int(self.window_seconds - (now - float(rows[0][0]))))
                connection.execute("COMMIT")
                return RateLimitResult(False, self.limit, 0, retry_after)
            connection.execute(
            ...
            used = len(rows) + 1
            connection.execute("COMMIT")
            return RateLimitResult(True, self.limit, max(0, self.limit - used), 0)
        except Exception:
            if connection.in_transaction:
                connection.execute("ROLLBACK")
            raise
