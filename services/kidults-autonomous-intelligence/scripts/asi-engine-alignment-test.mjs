...
assert.equal(registry.ASI_ENGINE_ALIGNMENT_PROFILE,'FOUR_PRINCIPLE_HARD_FLOOR_V2');
assert.equal(registry.ASI_ENGINE_ALIGNMENT_STATE,'ENFORCED');
assert.equal(Object.keys(registry.ASI_FLEET_LOGICAL_ENGINE).length,25);
assert.deepEqual(
  [...new Set(Object.values(registry.ASI_FLEET_LOGICAL_ENGINE))].sort(),
  [...registry.ASI_LOGICAL_ENGINES].sort(),
);
assert.ok(registry.ASI_FLEETS.every((fleet) => registry.ASI_FLEET_LOGICAL_ENGINE[fleet.id]));

const receipts = [];
for (const fleet of registry.ASI_FLEETS) {
  const logicalEngine = registry.ASI_FLEET_LOGICAL_ENGINE[fleet.id];
  ...
  assert.equal(first.logical_engine_id,logicalEngine);
...
const expiredReceipt = await alignment.assertAsiExecutionAlignment(fleetId,event({freshness_state:'EXPIRED'}));
assert.equal(expiredReceipt.principle_results.TRANSPARENT.state,'PASS');
