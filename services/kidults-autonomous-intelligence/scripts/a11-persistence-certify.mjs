...
const report = {
  ...
  startedAt,
  completedAt: nowIso(),
  configuration: { total, batchSize: BATCH_SIZE, readSamples: sampleCount, storage: 'SQLite WAL / D1-compatible persistence model', synthetic: true, productionEligible: false, databasePath: DB_PATH },
  persistence: { accepted, finalCount, countBeforeRestart, countAfterRestart, checkpointBeforeRestart, checkpointAfterRestart, duplicateLeakage, readMismatches, hashMismatches, aggregateGroups: aggregates.length },
  performance: {
    ...
    batchWriteLatencyMs: { p50: Number(percentile(writeLatencies, 50).toFixed(2)), p95: Number(percentile(writeLatencies, 95).toFixed(2)), p99: Number(percentile(writeLatencies, 99).toFixed(2)), max: Number(Math.max(...writeLatencies).toFixed(2)) },
    sampledReadLatencyMs: { p50: Number(percentile(readLatencies, 50).toFixed(4)), p95: Number(percentile(readLatencies, 95).toFixed(4)), p99: Number(percentile(readLatencies, 99).toFixed(4)), max: Number(Math.max(...readLatencies).toFixed(4)) },
    restartMs: Number(restartMs.toFixed(2)), aggregationMs: Number(aggregationMs.toFixed(2)), integritySweepMs: Number(integritySweepMs.toFixed(2)), memory: process.memoryUsage(),
  },
  gates,
};
