...
const forbiddenRuntimeImports=[
  './control-tower-gateway/control-tower-gateway.js','./control-tower-gateway/action-idempotency.js','./control-tower-gateway/action-lock.js','./control-tower-gateway/gateway-audit.js','./executive-orchestration/execution-orchestrator.js','./executive-orchestration/idempotency.js','./executive-orchestration/execution-lock.js'
];
...
const findings=[];
for(const specifier of forbiddenRuntimeImports) if(worker.includes(specifier)) findings.push(`LIVE_WORKER_IMPORTS_EPHEMERAL_STATE:${specifier}`);
...
const receipt={gate:'LIVE_RUNTIME_DURABLE_STATE_BOUNDARY',status:findings.length?'HOLD':'PASS',live_worker:'src/worker.ts',route_boundary:'src/control-tower-gateway/route-boundary.ts',durable_state_modules:durableStateModules.map(([relative])=>`src/${relative}`),forbidden_ephemeral_modules:forbiddenRuntimeImports,findings,production:'HOLD'};
