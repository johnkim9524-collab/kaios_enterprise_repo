...
export interface LockEntry { decisionId: string; lockedAt: string; lockedBy: string; }
export interface ExecutionLockStore { get(decisionId: string): LockEntry | undefined; set(decisionId: string, entry: LockEntry): void; delete(decisionId: string): void; }
export type LockAcquisitionResult = { acquired: true; lockEntry: LockEntry } | { acquired: false; reason: string; existingLock: LockEntry };
...
export function acquireExecutionLock(storeOrDecisionId: ExecutionLockStore | string, decisionIdOrAcquiredBy: string, acquiredByOrNowIso: string, maybeNowIso?: string): LockAcquisitionResult {
  if (typeof storeOrDecisionId === 'string') throw new Error('DURABLE_EXECUTION_LOCK_STORE_REQUIRED');
  ...
  if (existing) return { acquired:false, reason:`Decision ${decisionId} is already locked for execution by ${existing.lockedBy} since ${existing.lockedAt}. Concurrent execution rejected.`, existingLock:existing };
