...
export function checkIdempotency<T>(storeOrKey: IdempotencyStore | string, maybeKey?: string): IdempotencyCheckResult<T> {
  if (typeof storeOrKey === 'string') {
    throw new Error('DURABLE_IDEMPOTENCY_STORE_REQUIRED');
  }
  const key = maybeKey;
...
export function registerIdempotencyResult<T>(
  storeOrKey: IdempotencyStore | string,
  keyOrDecisionId: string,
  decisionIdOrResult: string | T,
  resultOrNowIso: T | string,
  maybeNowIso?: string,
): IdempotencyRecord<T> {
  if (typeof storeOrKey === 'string') {
    throw new Error('DURABLE_IDEMPOTENCY_STORE_REQUIRED');
