 ...
 * ring buffer or process-local counter is permitted. Legacy call shapes fail
 * closed until the governed gateway is connected to the control-plane audit
 * ledger.
 */
...
export interface GatewayAuditSink {
  append(event: GatewayAuditEvent): void;
}
...
export function recordAuditEvent(
  sinkOrParams: GatewayAuditSink | GatewayAuditParams,
  maybeParams?: GatewayAuditParams,
): GatewayAuditEvent {
...
export function getAuditLog(_limit = 100): readonly GatewayAuditEvent[] {
  throw new Error('DURABLE_GATEWAY_AUDIT_SINK_REQUIRED');
}
