...
export function evaluateRemoteActivation({ evidenceDir, manifestPath, expectedHeadSha }) {
  if (!evidenceDir || !manifestPath || !expectedHeadSha) {
    return { ok: false, reason: 'REMOTE_EVIDENCE_CONFIGURATION_REQUIRED' };
  }
  if (!/^[a-f0-9]{40}$/i.test(expectedHeadSha)) {
    return { ok: false, reason: 'EXPECTED_HEAD_SHA_INVALID' };
  }
  ...
  if (manifest.version !== 'remote-control-plane-evidence-manifest-v1') {
    return { ok: false, reason: 'REMOTE_EVIDENCE_MANIFEST_VERSION_INVALID' };
  }
  if (manifest.head_sha !== expectedHeadSha) {
    return { ok: false, reason: 'REMOTE_EVIDENCE_MANIFEST_STALE_HEAD' };
  }
  if (!Array.isArray(manifest.receipts)) {
    return { ok: false, reason: 'REMOTE_EVIDENCE_MANIFEST_RECEIPTS_INVALID' };
  }
  ...
  for (const [id, label] of REQUIRED_RECEIPTS) {
    ...
    if (sha256(raw) !== entry.sha256.toLowerCase()) {
      invalid.push({ label, reason: 'RECEIPT_DIGEST_MISMATCH' });
      continue;
    }
    if (receipt.id !== id) {
      invalid.push({ label, reason: 'RECEIPT_ID_MISMATCH' });
      continue;
    }
    if (receipt.status !== 'PASS') {
      invalid.push({ label, reason: 'RECEIPT_NOT_PASS' });
      continue;
    }
    if (receipt.head_sha !== expectedHeadSha) {
      invalid.push({ label, reason: 'RECEIPT_STALE_HEAD' });
      continue;
    }
  ...
  if (missing.length || invalid.length) {
    return { ok: false, reason: 'REMOTE_EVIDENCE_INCOMPLETE_OR_INVALID', missing, invalid };
  }
  return {
    ok: true,
    manifest_sha256: sha256(manifestRaw),
    verified_receipts: REQUIRED_RECEIPTS.map(([, label]) => label),
  };
}
...
function main() {
  ...
  console.log(JSON.stringify({
    ...
    governance_mode: 'SOLO_OWNER_AUTOMATED_EVIDENCE',
    required_human_reviewers: 0,
    state: 'ELIGIBLE_FOR_GOVERNED_CANARY',
    exact_head_sha: process.env.KPMO_EXPECTED_HEAD_SHA,
