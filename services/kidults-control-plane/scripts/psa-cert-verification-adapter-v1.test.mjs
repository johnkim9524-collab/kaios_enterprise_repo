...
const syntheticCertB = '1'.repeat(8);

test('PSA plan is bounded to three schema-only calls and contains no cert values', () => {
  const plan = buildPsaSchemaCanaryPlan({ certNumbers: [syntheticCertA, syntheticCertB], rightsReceipt: rights, asOf: '2026-08-27T00:00:00Z' });
  ...
  assert.throws(() => buildPsaSchemaCanaryPlan({ certNumbers: ['1','2','3','4'].map(value => value.repeat(4)), rightsReceipt: rights }), /ONE_TO_THREE/);
});
...
test('PSA schema canary never returns token, cert values, or raw payload', async () => {
  ...
  const receipt = await executePsaSchemaCanary({
    ...
    }, accessToken: token, certNumbers: [syntheticCertA], rightsReceipt: rights,
...
test('PSA failure still emits a bounded non-promotable receipt', async () => {
  const receipt = await executePsaSchemaCanary({
    ...
    accessToken: 'valid-token', certNumbers: [syntheticCertA], rightsReceipt: rights,
    now: () => new Date('2026-08-27T00:00:00Z'),
  });
...
test('PSA retry budget is bounded and preserves one call for each requested cert', async () => {
  ...
  const receipt = await executePsaSchemaCanary({
    ...
    sleep: async () => {}, accessToken: 'valid-token', certNumbers: [syntheticCertA, syntheticCertB], rightsReceipt: rights,
    now: () => new Date('2026-08-27T00:00:00Z'),
  });
...
test('PSA policy rejects public use and retention beyond thirty days', () => {
  assert.throws(() => buildPsaSchemaCanaryPlan({ certNumbers: [syntheticCertA], rightsReceipt: { ...rights, public_display: 'ALLOW' } }), /MUST_BE_BLOCKED/);
  assert.throws(() => buildPsaSchemaCanaryPlan({ certNumbers: [syntheticCertA], rightsReceipt: { ...rights, retention_days: 31 } }), /ONE_TO_THIRTY/);
});
