(() => {
  ...
  function lineChart(points) {
    ...
    return `<svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Kidult 100 staging trend"><defs><linearGradient id="trendFill" x1="0" x2="0" y1="0" y2="1"><stop offset="0" stop-color="#0b4a3b" stop-opacity=".36"/><stop offset="1" stop-color="#0b4a3b" stop-opacity=".02"/></linearGradient></defs><g class="chart-grid">${grid}</g><path d="${area}" fill="url(#trendFill)"/><path d="${path}" fill="none" stroke="#0b4a3b" stroke-width="3" vector-effect="non-scaling-stroke"/>${points.map((point, index) => `<circle cx="${x(index)}" cy="${y(point.value)}" r="5" fill="#f4efe7" stroke="#0b4a3b" stroke-width="3"><title>${esc(point.period)}: ${point.value}</title></circle>`).join('')}<g class="chart-labels">${labels}</g></svg>`;
  }

  function bars(items) {
    return items.map((item, index) => {
      const score = clampPercentage(item.score);
      return `<article class="category-row"><div class="rank">${String(index + 1).padStart(2, '0')}</div><div class="category-copy"><div><span>${esc(item.name)}</span><strong>${Number(item.score).toFixed(1)}</strong></div><div class="bar-track" role="img" aria-label="${esc(item.name)} score ${score} out of 100"><i style="width:${score}%"></i></div><div class="category-meta"><span>Confidence ${item.confidence}</span><span>Velocity ${Number(item.velocity).toFixed(1)}</span><span>Liquidity ${item.liquidity}</span><b>${esc(item.state)}</b></div></div></article>`;
    }).join('');
  }
  ...
  function evidenceComposition(items) {
    ...
    const normalizedItems = items.map((item, index) => ({ ...item, color: colors[index % colors.length], value: Number(item.value) || 0 }));
    const stacked = normalizedItems.map((item) => `<span style="width:${total > 0 ? (item.value / total) * 100 : 0}%;background:${item.color}" title="${esc(item.name)}: ${item.value}%"></span>`).join('');
    const legend = normalizedItems.map((item) => `<span><i style="background:${item.color}"></i>${esc(item.name)} <b>${item.value}%</b></span>`).join('');
    return `<div class="composition-summary"><div class="composition-stack" role="img" aria-label="Evidence dimension composition totals ${total}%">${stacked}</div><div class="composition-total"><strong>${total}%</strong><span>Total composition</span></div></div><div class="composition-legend">${legend}</div><div class="hbar-chart composition-detail">${horizontal(items, 'value', 'name', '%', 'percent')}</div>`;
  }
  ...
  function matrix(data) {
    return `<div class="matrix-grid" style="--count:${data.labels.length + 1}"><span></span>${data.labels.map((label) => `<b>${esc(label)}</b>`).join('')}${data.labels.map((label, row) => `<b>${esc(label)}</b>${data.values[row].map((value) => `<span style="--alpha:${Math.max(.1, value)}"><i>${value.toFixed(2)}</i></span>`).join('')}`).join('')}</div>`;
  }
  ...
  fetch(dataAsset, { cache: 'no-store' })
    ...
    .then((data) => {
      ...
      $$('[data-status-label]').forEach((node) => {
        node.textContent = previewMode ? `${data.label} · PREVIEW` : data.label;
      });
      document.documentElement.dataset.dataReady = 'true';
      document.dispatchEvent(new CustomEvent('kidults:data-loaded', { detail: { mode: previewMode ? 'preview' : 'live', asset: dataAsset } }));
    })
    .catch((error) => {
      console.error('Intelligence data load failed', error);
      $$('[data-status-label]').forEach((node) => {
        node.textContent = previewMode ? 'Preview data unavailable' : 'Data temporarily unavailable';
      });
