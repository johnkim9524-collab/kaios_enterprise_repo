...
All registered secret-bearing jobs implement the same inline, secret-free preflight before any provider credential is resolved. The preflight uses `${{ github.token }}` with exact registry-bound read permissions to GET `/repos/$GITHUB_REPOSITORY/branches/main`; an unreadable or malformed response, a non-`main` ref, or any SHA other than exact `GITHUB_SHA` fails closed. The default permission set is only `contents: read`. The Production evidence lane is the sole registered exception: it adds only `actions: read`, and its GitHub-token consumers are exactly the live-main guard, evidence-run/artifact provenance check, and pinned artifact download. Any extra permission, lane, or token-consuming step fails closed.

Provider secrets are forbidden at workflow and job scope. The dispatch registry names every allowed credential-consuming step for each privileged job, and the repository validator proves that guard and step-scope counts are derived from the registry, with zero workflow-scope bindings, zero job-scope bindings, and rejects stale, unreadable, non-main, token-permission, token-consumer, activation-receipt body/order, live-main order, and scope mutations.
...
The remote PostgreSQL/PITR lane is bound as `REMOTE_STAGING_MUTATION` and is dispatch-only. Each source or restore run must bind an exact current-main SHA, a UUIDv4 nonce, and a short expiry, then pass the existing `kidults-do-staging-ssh` Environment review before any secret-consuming step. There is no standing repository-variable activation and no `push` execution path. Missing per-run authority skips the guarded job; missing DSN fails before any remote file write. This repository control does not itself authorize STAGING, create a credential, prove PostgreSQL/PITR readiness, or change Production/Public/G5.
