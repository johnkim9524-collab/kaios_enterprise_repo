...
The collector derives the current privileged-job set from `secret-bearing-workflow-dispatch-registry-v1.json`, parses each job's secret references and Environment binding, and performs GET-only GitHub metadata reads. Every list endpoint is paginated to exhaustion and reconciled to its reported count before it can contribute to a pass. Every listed ruleset also requires a readable, identity-matched detail GET. The sanitized ruleset evidence retains repository `source`/`source_type`, exact ref include/exclude arrays, bypass count without actor identity, strict-status policy, and each required status context with its numeric GitHub Actions integration ID. The receipt separately verifies that `KAIOS Solo Owner Preflight` binds the three technical base contexts plus `KIDULTS Governed Landing Authorization V1` and `KIDULTS Scope-Aware Authoritative Status V1` to integration `15368`; missing, ambiguous, non-strict, bypassed, wrong-source, wrong-ref, or integration-drifted read-back blocks the receipt. These fields are material to the read-back digest. The receipt also records exact source SHA/ref, live default-branch SHA, endpoint statuses, registry-to-workflow binding results, deployment-policy results, Environment coverage, repository/organization scope-absence counts/digests, and negative-execution evidence without emitting secret names or values.
...
- a truncated ruleset list, unreadable or identity-mismatched ruleset detail, wrong repository source, broadened/default-branch ref condition, bypass actor, non-strict required status policy, missing native context, duplicate context, or wrong `integration_id`;
...
The current registry covers secret-bearing jobs regardless of whether they are activated by `push`, `schedule`, `workflow_run`, reusable-workflow handoff, or `workflow_dispatch`. Each binding records allowed trigger classes and a mutation class. A trigger transformation without an atomic registry migration fails closed.

The remote PostgreSQL/PITR lane is bound as `REMOTE_STAGING_MUTATION`. `KIDULTS_REMOTE_POSTGRES_AUTO_ACTIVATION_AUTHORIZED` must remain the literal string `false`; `true` and missing values both block the privileged lane. A separate ledger-only job must atomically consume one ACTIVE external grant bound to exact operation, live-main SHA, workflow, run, attempt, fresh UUID, ledger expiry and target digest. The provider job starts only from that verified `CONSUMED` receipt and rechecks the signed expiry before its first step-scoped SSH/PostgreSQL secret. Replay, consumed, expired, mismatched, offline and ambiguous ledger outcomes produce zero provider credential use. This repository control does not itself authorize STAGING, create a credential, prove PostgreSQL/PITR readiness, or change Production/Public/G5.
