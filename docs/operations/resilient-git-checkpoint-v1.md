...
After checkpoint creation, the governed bundle runner combines target preflight, KMS-encrypted
bundle and manifest upload, exact-version provider read-back, SHA-256 comparison, Ed25519 KMS
receipt signing, and an isolated restore test into one fail-closed batch:

```sh
AWS_PROFILE=<sso-profile> npm run run:decade-durability-batch -- \
  --checkpoint-root /absolute/protected/kidults-git-checkpoints \
  --receipt-root /absolute/protected/kidults-durability-receipts \
  --branch codex/common-control-foundation-v1 \
  --bucket <object-lock-bucket> --region ap-northeast-2 \
  --kms-key-arn <s3-encryption-key-arn> \
  --signing-key-arn <ed25519-signing-key-arn> \
  --provider-domain <independent-provider-domain> \
  --admin-domain <independent-administrative-domain>
```

The runner never grants promotion or release authority. It refuses relative local paths,
same-provider or primary-admin labels, invalid checkpoint digests, non-COMPLIANCE retention,
missing S3 version IDs, checksum mismatches, signature failures, restored SHA/tree mismatch, and
failed governance tests. Offline media and independent-custodian annual restore remain separate
gates and therefore remain `HOLD` until their own evidence exists.

## Integrated durability operations

The seven operating capabilities are governed as one fail-closed loop by
`kidults-decade-durability-operations-contract-v1`: checkpoint orchestration, restore drills,
identity and key recovery, durability classification, observability and alerting, an evidence-only
dashboard, and the operating-transition gate. The evaluator accepts only exact, versioned evidence
and never grants AWS writes, Production, Public, or G5 authority:

```sh
npm run assess:durability-operations -- \
  --contract coordination/kidults/governance/decade-durability-operations-contract-v1.json \
  --evidence /absolute/protected/durability-operations-evidence.json \
  --now 2026-09-20T00:00:00.000Z
```

The daily GitHub workflow validates the deterministic control plane. It deliberately does not hold
cloud credentials or simulate external evidence. AWS Object Lock writes, offline media writes,
independent-custodian drills, identity recovery tests, and all release transitions remain separate
evidence-producing operations.

### Encrypted offline cold copy

Prepare a 32-byte encryption key and an Ed25519 signing key outside the removable medium. Never
place either private key on the USB device. The governed offline runner writes a content-addressed
AES-256-GCM generation, reads it back, verifies its detached Ed25519 signature, decrypts it only
inside an isolated temporary directory, checks the exact source SHA/tree, and runs the governance
recovery tests:

```sh
openssl rand 32 > /absolute/separate-key-store/offline-aes256.key
openssl genpkey -algorithm ED25519 \
  -out /absolute/separate-key-store/offline-signing-ed25519.pem

npm run offline:prepare-cold-copy -- \
  --checkpoint-root /absolute/protected/kidults-git-checkpoints \
  --media-root /absolute/removable-media \
  --key-file /absolute/separate-key-store/offline-aes256.key \
  --signing-key-file /absolute/separate-key-store/offline-signing-ed25519.pem \
  --branch codex/common-control-foundation-v1 \
  --custodian-id <separate-custodian-id> --media-id <physical-media-id>

npm run offline:verify-cold-copy -- \
  --checkpoint-root /absolute/protected/kidults-git-checkpoints \
  --media-root /absolute/removable-media \
  --key-file /absolute/separate-key-store/offline-aes256.key \
  --signing-key-file /absolute/separate-key-store/offline-signing-ed25519.pem \
...
The local verification intentionally reports `independentCustodian: false`. It proves the medium,
cryptography, exact read-back, and isolated restore, but it cannot self-attest that another human
or administrative domain performed the annual independent restore. That final attestation remains
`HOLD` until the separate custodian actually executes and signs the independent drill.

Every object step is interruption-safe. Before uploading an immutable key, the runner queries its
current exact S3 version. If present, it reuses that version only after verifying the expected KMS
key, source SHA/tree metadata, SHA-256 provider checksum, byte digest, and COMPLIANCE retention.
A current delete marker, ambiguous version state, or any control mismatch fails closed instead of
creating another immutable version. This permits a partial bundle/manifest upload to resume without
duplicating locked objects.
