...
const mutationCases = [
  ...
  ['restore target time omitted from target binding', (x) => { x.restoreText = x.restoreText.replace("              'restore_target_time':fixture['target_time'],\n", ''); }],
  ['restore target time provider recheck omitted', (x) => { x.restoreText = x.restoreText.replaceAll("              'restore_target_time':os.environ['RESTORE_TARGET_TIME'],\n", ''); }],
  ...
  ['restore target time contract field removed', (x) => { x.contract.operations[1].target_binding.required_fields = x.contract.operations[1].target_binding.required_fields.filter((field) => field !== 'restore_target_time'); }],
  ...
  ['terminal receipt job removed', (x) => { x.sourceText = x.sourceText.replace('  terminal-receipt:', '  removed-terminal-receipt:'); }],
  ['terminal receipt no longer always runs', (x) => { x.restoreText = x.restoreText.replace('    if: ${{ always() }}', "    if: success()"); }],
  ['terminal receipt file cardinality weakened', (x) => { x.sourceText = x.sourceText.replace('test "$(find "$ROOT" -type f -name \'*.json\' | wc -l)" -eq 1', 'true'); }],
  ['terminal receipt nonempty guard removed', (x) => { x.restoreText = x.restoreText.replace('          test -s "$RECEIPT"\n', ''); }],
  ['terminal receipt exposes raw approval id', (x) => { x.sourceText = x.sourceText.replace("              'approval_id_sha256':", "              'approval_id':approval,\n              'approval_id_sha256':"); }],
  ['terminal receipt drops production hold', (x) => { x.restoreText = x.restoreText.replace("              'terminal_receipt_cardinality':1,\n              'production':'HOLD',", "              'terminal_receipt_cardinality':1,\n              'production':'READY',"); }],
