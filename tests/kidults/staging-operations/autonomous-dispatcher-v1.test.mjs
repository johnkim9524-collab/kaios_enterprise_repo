...
assert.match(dispatcherWorkflow,/READINESS_WORKFLOW_NAME: \$\{\{ github\.event\.workflow_run\.name \}\}/);
assert.match(dispatcherWorkflow,/toJson\(github\.event\.workflow_run\.pull_requests\)/);
assert.match(dispatcherWorkflow,/Bind exact workflow_run PR identity before authority consumption/);
assert.match(dispatcherWorkflow,/jq -e --argjson repo_id "\$GITHUB_REPOSITORY_ID" 'type=="array" and length>0 and \.\[0\]\.head\.repo\.id == \$repo_id'/);
assert.match(dispatcherWorkflow,/pr_number=\$\(jq -r 'if type=="array" and length>0 then \.\[0\]\.number \/\/ empty else empty end'/);
assert.match(dispatcherWorkflow,/head_sha=\$\(jq -r 'if type=="array" and length>0 then \.\[0\]\.head\.sha \/\/ empty else empty end'/);
assert.match(dispatcherWorkflow,/dispatch_eligible=\$\(jq -r '\.dispatch_eligible \/\/ false'/);
...
assert.match(dispatcherWorkflow,/READINESS_PR_NUMBER: \$\{\{ steps\.bind_workflow_run\.outputs\.pr_number \|\| github\.event\.pull_request\.number \}\}/);
assert.match(dispatcherWorkflow,/READINESS_HEAD_SHA: \$\{\{ steps\.bind_workflow_run\.outputs\.head_sha \|\| github\.event\.pull_request\.head\.sha \}\}/);
assert.match(dispatcherWorkflow,/steps\.consume_readiness\.outputs\.pull_request \|\| steps\.bind_workflow_run\.outputs\.pr_number \|\| github\.event\.pull_request\.number \|\| inputs\.pull_request/);
