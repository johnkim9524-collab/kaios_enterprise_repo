...
function runCase(id, {apiToken, accountId, expectedPresence}) {
  ...
  const result = spawnSync('bash', [script], {
    ...
    env: {...process.env,CLOUDFLARE_API_TOKEN:apiToken,CLOUDFLARE_ACCOUNT_ID:accountId,
      CLOUDFLARE_PAGES_PROJECT_NAME:'kidults-workspace-staging',EXPECTED_REPOSITORY:'johnkim9524-collab/kaios_enterprise_repo',
      RECEIPT_DIR:receiptDir,GITHUB_SHA:'1111111111111111111111111111111111111111'}
  });
  ...
  assert.doesNotMatch(raw, /test-token-never-real|test-account-never-real/);
}

runCase('token-absent',{apiToken:'',accountId:'test-account-never-real',expectedPresence:{api_token_present:false,account_id_present:true}});
runCase('account-absent',{apiToken:'test-token-never-real',accountId:'',expectedPresence:{api_token_present:true,account_id_present:false}});
runCase('both-absent',{apiToken:'',accountId:'',expectedPresence:{api_token_present:false,account_id_present:false}});

console.log(JSON.stringify({suite:'KIDULTS_CLOUDFLARE_PAGES_READONLY_CREDENTIAL_FAILURE_RECEIPT_V1',result:'PASS',cases:3,fail_closed_exit:65,sanitized_receipt_emitted:true,cloudflare_api_called:false,remote_mutation:false,public_release:'HOLD',production:'HOLD',g5:'HOLD'},null,2));
