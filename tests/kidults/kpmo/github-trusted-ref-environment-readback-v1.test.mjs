...
test('current exact registry binds all 14 privileged lanes but external policy remains fail closed', () => {
  assert.equal(currentInventory.registered_lane_count, 14);
  assert.equal(currentInventory.secret_bearing_job_count, 14);
...
test('all 14 privileged jobs reject unreadable, stale, and non-main live-main guards', () => {
  const mutations = [
    [
...
test('all 14 privileged jobs reject secret scope and guard order mutations', () => {
  let rejected = 0;
  for (const lane of currentInventory.lanes) {
