...
def validate_contract(text: str) -> None:
    ...
    assert 'terminal_readback_bound:{attempts:24,sleep_seconds:5}' in text
    assert 'for readback_attempt in $(seq 1 24); do' in text
...
def test_passive_wait_shrinkage_is_rejected() -> None:
    text = WORKFLOW_PATH.read_text(encoding="utf-8")
    mutated = text.replace('for readback_attempt in $(seq 1 24); do', 'for readback_attempt in $(seq 1 4); do', 1)
