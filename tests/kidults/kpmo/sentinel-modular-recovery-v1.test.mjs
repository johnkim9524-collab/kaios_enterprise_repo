...
import {readStableSentinelSnapshot} from '../../../scripts/kidults/kpmo/read-stable-sentinel-snapshot-v1.mjs';
...
// Read retries are local observations, never workflow or authorization retries.
test('stable first snapshot uses one read and no delay', async () => {
  let calls = 0;
  const expected = {generation: 5};
  const value = await readStableSentinelSnapshot(async () => {calls += 1; return expected;}, {
    onRetry: () => assert.fail('no retry expected'), wait: () => assert.fail('no wait expected'),
  });
  assert.strictEqual(value, expected); assert.equal(calls, 1);
});

test('transient change discards the entire old snapshot and reports the reread', async () => {
  const traces = [], waits = [];
  let calls = 0;
  const fresh = {runs: {SHADOW: [{id: 20}]}, archives_by_id: {20: 'fresh'}};
  const result = await readStableSentinelSnapshot(async () => {
    calls += 1;
    if (calls === 1) throw new Error('SENTINEL_GENERATION_CHANGED_DURING_READ');
    return fresh;
  }, {onRetry: x => traces.push(x), wait: async ms => waits.push(ms)});
  assert.strictEqual(result, fresh); assert.equal(calls, 2);
  assert.deepEqual(waits, [100]); assert.equal(traces.length, 1);
  assert.equal(traces[0].cached_snapshot_reused, false);
  assert.equal(traces[0].next_attempt, 2); assert.ok(Object.isFrozen(traces[0]));
});

test('three unstable reads terminate with the original failure; no endless loop', async () => {
  let calls = 0, waits = 0;
  const error = new Error('SENTINEL_GENERATION_CHANGED_DURING_READ');
  await assert.rejects(readStableSentinelSnapshot(async () => {calls += 1; throw error;}, {wait: async () => {waits += 1;}}), e => e === error);
  assert.equal(calls, 3); assert.equal(waits, 2);
});

for (const code of [
  'SENTINEL_MAIN_CHANGED_DURING_READ', 'SENTINEL_EXACT_LIVE_MAIN_MISMATCH',
  'COVERAGE_ALIAS_RUN_CHANGED_DURING_READ', 'COVERAGE_ALIAS_REMOTE_BINDING',
  'GITHUB_HTTP_401', 'GITHUB_HTTP_403', 'GITHUB_HTTP_429', 'GITHUB_READ_TRANSPORT_FAILED',
  'REQUIREMENT_RUN_INDEX_COUNT_CHANGED', 'RUN_INDEX_DUPLICATE_ID',
  'RUN_INDEX_TERMINAL_CONCLUSION_INVALID', 'ARTIFACT_INDEX_TRUNCATED',
  'ARTIFACT_DOWNLOAD_TRANSPORT_FAILED', 'SENTINEL_GENERATION_CHANGED_DURING_READ:EXTRA',
  'ATOMIC_LANDING_AUTHORIZATION_ALREADY_CONSUMED',
]) {
  test(`no retry or weakening for ${code}`, async () => {
    let calls = 0;
    await assert.rejects(readStableSentinelSnapshot(async () => {calls += 1; throw new Error(code);}, {
      wait: () => assert.fail('forbidden retry'),
    }), e => e.message === code);
    assert.equal(calls, 1);
  });
}

for (const verdict of ['VERIFIED_FAIL', 'VERIFIED_HOLD']) {
  test(`a stable semantic ${verdict} is returned as-is, never retried into green`, async () => {
    let calls = 0;
    const value = {state: verdict, failed_producers: ['REQUIREMENT']};
    const actual = await readStableSentinelSnapshot(async () => {calls += 1; return value;});
    assert.strictEqual(actual, value); assert.equal(calls, 1);
  });
}
for (const bound of [0, -1, 4, 1.5, '3']) {
  test(`invalid retry bound ${JSON.stringify(bound)} fails before reading`, async () => {
    await assert.rejects(readStableSentinelSnapshot(() => assert.fail('must not read'), {maxAttempts: bound}), /SENTINEL_SNAPSHOT_ATTEMPT_BOUND/);
  });
}
for (const bound of [-1, 1001, 0.5]) {
  test(`invalid delay bound ${bound} fails before reading`, async () => {
    await assert.rejects(readStableSentinelSnapshot(() => assert.fail('must not read'), {retryDelayMs: bound}), /SENTINEL_SNAPSHOT_DELAY_BOUND/);
  });
}

test('single-attempt mode never retries', async () => {
  let calls = 0;
  await assert.rejects(readStableSentinelSnapshot(async () => {calls += 1; throw new Error('SENTINEL_GENERATION_CHANGED_DURING_READ');}, {maxAttempts: 1}), /SENTINEL_GENERATION_CHANGED_DURING_READ/);
  assert.equal(calls, 1);
});

test('non-Error throw is not treated as a retryable consistency error', async () => {
  let calls = 0;
  await assert.rejects(readStableSentinelSnapshot(async () => {calls += 1; throw 'SENTINEL_GENERATION_CHANGED_DURING_READ';}), e => e === 'SENTINEL_GENERATION_CHANGED_DURING_READ');
  assert.equal(calls, 1);
});

test('bad callback is rejected', async () => {
  await assert.rejects(readStableSentinelSnapshot(null), /SENTINEL_SNAPSHOT_CALLBACK_INVALID/);
});
...
function harness(t, scenario) {
  ...
  for (const name of [
    'sentinel-health-receipt-contract-v1.mjs', 'read-stable-sentinel-snapshot-v1.mjs',
...
test('actual Sentinel CLI retries a changing read once, then preserves nonterminal HOLD', t => {
  const {result, receipt, trace} = harness(t, 'race-once').run();
  assert.equal(result.status, 1);
  assert.equal(receipt.state, 'VERIFIED_HOLD');
  assert.equal(receipt.producers[0].selected_run_id, 101);
  assert.equal(receipt.producers[0].state, 'VERIFIED_HOLD');
  assert.equal((result.stderr.match(/SENTINEL_SNAPSHOT_DISCARDED_READING_FRESH/g) || []).length, 1);
  assert.equal(trace.split('\n').filter(x => x.endsWith('/branches/main')).length, 3);
  assert.doesNotThrow(() => validateHealthReceipt(receipt, env));
});

test('actual Sentinel CLI persistent race ends after three full collections with a valid failure', t => {
  const {result, receipt, trace} = harness(t, 'race-always').run();
  assert.equal(result.status, 1); assert.equal(receipt.state, 'VERIFIED_FAIL');
  assert.equal(receipt.failure_class, 'SENTINEL_GENERATION_CHANGED_DURING_READ');
  assert.equal((result.stderr.match(/SENTINEL_SNAPSHOT_DISCARDED_READING_FRESH/g) || []).length, 2);
  assert.equal(trace.split('\n').filter(x => x.endsWith('/branches/main')).length, 3);
  assert.doesNotThrow(() => validateHealthReceipt(receipt, env));
});
